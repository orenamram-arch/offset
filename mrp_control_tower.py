...
""")
    ...
    if not _bom_positions:
        ...
        for parent_pn, children_pns in ASSEMBLY_CHILDREN.items():
            ...
            for c in valid_children:
                ...
                qty_lbl = ASSEMBLY_BOM_TREE.get(c, {}).get("qty_per_parent", 1.0) or 1.0
    ...
    # מה שרואים בעץ.
    _cp_asm_node_by_root = {}  # root_pn -> {asm_pn: node}
    for _root, _res in _cp_results.items():
        _by_pn = {}
        for _node in _res["nodes"].values():
            if _node["kind"] == "assembly":
                _prev = _by_pn.get(_node["pn"])
                if _prev is None or _node["ef"] > _prev["ef"]:
                    _by_pn[_node["pn"]] = _node
        _cp_asm_node_by_root[_root] = _by_pn

    def _cp_find_node_for_assembly(asm_pn):
        """מוצא את הצומת המייצג של הרכבה נתונה - מחפש בעץ של השורש שאליו היא
        שייכת (לפי ASSEMBLY_BOM_TREE), כדי להציג לה תאריך/Slack עקביים עם
        מה שבאמת חושב לה בתוך העץ המלא שלה."""
        _cur = asm_pn
        _guard = 0
        while ASSEMBLY_BOM_TREE.get(_cur, {}).get("parent") is not None and _guard < 200:
            _cur = ASSEMBLY_BOM_TREE[_cur]["parent"]
            _guard += 1
        _root_of = _cur
        return _cp_asm_node_by_root.get(_root_of, {}).get(asm_pn)
        ...
        while _nid is not None and _depth < max_depth:
            ...
            if _node["kind"] == "component":
                if _node.get("availability_reason") == "stock":
                    ...
                    _eta_note = " (הנחה - אין ETA)" if _node.get("is_eta_assumption") else ""
                    _parts.append(f"{_node['pn']} — {str(_node['desc'])[:30]}{_eta_note}")
                break
            if not _node["children_ids"]:
    ...
    for _asm in valid_assemblies:
        ...
        _root_of_asm = _asm
        _guard2 = 0
        while ASSEMBLY_BOM_TREE.get(_root_of_asm, {}).get("parent") is not None and _guard2 < 200:
            _root_of_asm = ASSEMBLY_BOM_TREE[_root_of_asm]["parent"]
            _guard2 += 1
        _date_str = _node["ef"].strftime("%Y-%m-%d") + (" (הנחה)" if _node.get("is_eta_assumption") else "")
        _cp_rows.append({
            "קוד הרכבה": _asm,
            ...
            "_sort_date": _node["ef"],
            "מבוסס על הנחה": "כן (הנחה)" if _node.get("is_eta_assumption") else "לא (ETA אמיתי)",
    ...
    with tab_tree:
        ...
        for _root in sorted(_cp_roots):
            ...
            with st.expander(f"🔗 {_root} — {_root_desc}  |  יעד: {_chain[0]['ef'].strftime('%Y-%m-%d')}", expanded=True):
                ...
                for _i, _cnode in enumerate(_chain):
                    ...
                    _cdate = _cnode["ef"].strftime("%Y-%m-%d") + (" (הנחה)" if _cnode.get("is_eta_assumption") else "")
        ...
        def _render_cp_node(node_id, root_pn, depth):
            ...
            date_str = node["ef"].strftime("%Y-%m-%d") + (" (הנחה)" if node.get("is_eta_assumption") else "")
