...
""")
    ...
    if not _bom_positions:
        ...
        UNIT_PX_X = 170
        UNIT_PX_Y = 130
        BOX_W, BOX_H = 0.88, 0.66
        BOX_W_PX = BOX_W * UNIT_PX_X
        PN_CHARS_PER_LINE = max(6, int(BOX_W_PX / 8.2))     # גופן מודגש וגדול יותר -> פחות תווים לשורה
        DESC_CHARS_PER_LINE = max(8, int(BOX_W_PX / 6.0))   # גופן קטן יותר -> יותר תווים לשורה
        ...
        for pn, info in ASSEMBLY_BOM_TREE.items():
            ...
            desc_lines = _textwrap.wrap(desc_raw, width=DESC_CHARS_PER_LINE, max_lines=2, placeholder="…") if desc_raw else []

            pn_html = "<br>".join(pn_lines)
            desc_html = "<br>".join(desc_lines)
            full_text = f"<b>{pn_html}</b>" + (f"<br>{desc_html}" if desc_html else "")
    ...
    def _cp_get_component_eta_date(comp_pn):
        ...
        if eta_str and eta_str != "בדיקה נדרשת":
            try:
                return pd.to_datetime(eta_str), False
            except Exception:
                pass
        return _assumed_date, True
    ...
    def _cp_compute(asm_pn, visited):
        ...
        limiting_type, limiting_ref, limiting_display = None, None, None
        ...
        if asm_pn in df.columns:
            for _, row in df.iterrows():
                ...
                if latest_date is None or comp_date > latest_date:
                    ...
                    limiting_display = f"{comp_pn} ({str(row[DESC_COL])[:24]})"

        for child_pn in ASSEMBLY_CHILDREN.get(asm_pn, []):
            child_date, child_assumed, _, _, child_display = _cp_compute(child_pn, visited)
        ...
        if latest_date is None:
            ...
            limiting_type, limiting_ref, limiting_display = None, None, "אין נתוני רכיבים"
    ...
    def _build_critical_chain(root_pn):
        ...
        while current is not None and guard < 200:
            chain.add(current)
            _, _, ltype, lref, _ = _cp_cache.get(current, (None, False, None, None, None))
    ...
    _cp_rows = []
    for _asm in valid_assemblies:
        _date, _assumed, _ltype, _lref, _ldisp = _cp_cache.get(_asm, (None, False, None, None, None))
        _cp_rows.append({
            "קוד הרכבה": _asm,
    ...
    with tab_tree:
        ...
        def _render_cp_node(asm_pn, depth):
            ...
            date, assumed, _, _, _ = _cp_cache.get(asm_pn, (None, False, None, None, None))
            ...
            if children:
                ...
                st.markdown(
                    ...
                    f"border-right:3px solid {'%s' % DANGER if is_critical else 'rgba(128,128,128,0.3)'};'>{label}</div>",
                    unsafe_allow_html=True
                )
