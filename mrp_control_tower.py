...
def calculate_critical_path(root_asm_pn, assembly_children=None, assembly_direct_components=None,
                             get_eta_func=None, inv_cache=None, shortage_pns=None, today=None):
    ...
    פרמטרים:
      ...
      inv_cache                   - מטמון רשומות מלאי, מועבר ל-get_first_supply_eta כברירת המחדל.
      shortage_pns                - set אופציונלי של מק"טים שבאמת בחוסר בפועל.
                                     אם סופק - רכיב שלא ברשימה נחשב זמין כבר היום
                                     (יש ממנו מספיק מלאי) גם אם רשום לו ETA עתידי -
                                     הוא לא באמת חוסם. אם None, כל רכיב נבדק לפי
                                     ה-ETA שלו בפועל בלי הסינון הזה.
    ...
    שיטת החישוב - CPM קלאסי, שני מעברים על העץ:
      1. Forward Pass (Post-order - הילדים תמיד מחושבים לפני ההורה):
         - צומת-רכיב-קצה (עלה בעץ ה-BOM - אין לו בנים משלו, כלומר רכיב
           רכש/חומר גלם אמיתי): ES=today, EF=ETA האפקטיבי (או
           today+60 יום אם אין ETA תקין/הרכיב "בדיקה נדרשת").
    ...
    def _effective_component_date(comp_pn):
        """תאריך זמינות אפקטיבי לרכיב רכש - ראה shortage_pns בדוקסטרינג למעלה.
        ...
        """
        if shortage_pns is not None and comp_pn not in shortage_pns:
            return today, False
...
""")
    ...
    # תיקון קריטי (סבב 2): לא משתמשים ב-breakdown_df כמות שהוא - הוא מוגבל
    # ל-selected_target_yms (חלון "כמה חודשים קדימה" שנבחר בפילטרים של טאב
    # אחר!). אבל גם לא בודקים חוסר על פני *כל* אופק התכנון - זה תפס בטעות
    # רכיבים עם מלאי מספיק לבנייה הקרובה, אבל שמחושב להם חוסר רק אחרי כמה
    # חודשי צריכה מצטברת רחוק בעתיד (לא רלוונטי לשאלה "האם אפשר להתחיל
    # לבנות עכשיו"). נתיב קריטי נבדק תמיד ביחס ל**תוכנית העבודה הראשונה**
    # (החודש המוקדם ביותר בכל תוכנית העבודה של הפרויקט, all_ym_list[0]) -
    # בלי שום תלות בפילטרים של טאב אחר (לא בחלון "כמה חודשים קדימה" ולא
    # בסוג פריט/הרכבה/חיפוש שרלוונטיים רק לתצוגה האינטראקטיבית של טבלת
    # החוסרים) - לכן קוראים ישירות ל-calculate_mrp_breakdown_cached (אותו
    # מנגנון קאש בדיוק), לא ל-wrapper.
    _cp_first_ym = [all_ym_list[0]] if all_ym_list else selected_target_yms
    _cp_full_breakdown_df = calculate_mrp_breakdown_cached(tuple(_cp_first_ym), tuple(), assembly_plan_df)
    _cp_shortage_pns = set(_cp_full_breakdown_df["PN"].unique()) if not _cp_full_breakdown_df.empty else set()
    st.caption(f"🔎 מבוסס על {len(_cp_shortage_pns)} רכיבים שנמצאים בחוסר אמיתי ביחס לתוכנית העבודה הראשונה של הפרויקט ({_cp_first_ym[0] if _cp_first_ym else '—'}) - לא תלוי בפילטרים של שאר האפליקציה.")
