...
""")
    ...
    # ==========================================================
    # לכל הרכבה: תאריך המוכנות שלה = המקסימום (המאוחר ביותר) מבין -
    # (א) ה-ETA של כל רכיבי הגלם הישירים שלה, ו-(ב) תאריך המוכנות
    # (רקורסיבי) של כל תתי-ההרכבות שלה - כלומר "מי מגיע הכי מאוחר
    # בכל השרשרת שמתחת להרכבה הזו" הוא זה שקובע מתי היא בעצמה מוכנה.
    # לפריט בלי ETA בכלל (למשל "בדיקה נדרשת") - מניחים חודש אחד קדימה
    # מהיום, ומסמנים "(הנחה)" בפירוש בתוצאה, כדי שלא יתבלבל עם ETA אמיתי.
    #
    # תיקון (בעקבות משוב): (1) הנחת ברירת המחדל לפריט בלי ETA שונתה
    # מ-4 חודשים לחודש אחד. (2) נוסף עץ אינטראקטיבי הניתן לפתיחה/סגירה
    # ברמה-ברמה (expander מקונן), עם סימון אדום ברור על ההרכבות שנמצאות
    # בפועל על נתיב החסימה (כלומר: הן, או שרשרת ילד-אחר-ילד מתחתן, הן
    # שקובעות את תאריך המוכנות של ראש העץ) - כדי לראות מיד "מי אשם"
    # בעיכוב, לא רק את התאריך הסופי.
    ...
    # תיקון קריטי (בעקבות משוב מפורט): הגרסה הקודמת השתמשה ב-ETA של **כל**
    # רכיב גלם ישיר, גם רכיבים שיש מהם מלאי מספיק ואינם בחוסר בפועל -
    # מה שיכול היה למשוך את התאריך קדימה בטעות בגלל הזמנה עתידית
    # שגרתית שלא בכלל דחופה (כי המלאי הקיים כבר מספיק). התיקון: משתמשים
    # ב-breakdown_df הקיים (בדיוק אותה רשימת החוסרים שמוצגת בכל שאר
    # האפליקציה - כרגע X רכיבים) כדי לדעת אילו רכיבים באמת חוסמים; רכיב
    # שלא ברשימה נחשב זמין כבר היום. בנוסף, נוסף זמן בנייה קבוע (2 ימים)
    # לכל הרכבה בעץ - מתווסף מעל הזמינות המקסימלית של הרכיבים/תתי-
    # ההרכבות שלה, ומצטבר כלפי מעלה עם כל רמה נוספת בעץ.
    _cp_inv_cache = fetch_all_inventory_records(PROJECT_ID)
    _ASSUMED_MONTHS_AHEAD = 1
    _BUILD_DAYS_PER_ASSEMBLY = 2
    _today_ts = pd.Timestamp.today().normalize()
    _assumed_date = _today_ts + pd.DateOffset(months=_ASSUMED_MONTHS_AHEAD)
    ...
    _cp_cache = {}  # asm_pn -> (latest_date, is_assumption, limiting_type, limiting_ref, limiting_display, limiting_comp_desc)
    _cp_eta_cache = {}  # comp_pn -> (date, is_assumption) - רכיב משותף בין הרכבות נבדק פעם אחת בלבד

    def _cp_get_component_eta_date(comp_pn):
        if comp_pn in _cp_eta_cache:
            return _cp_eta_cache[comp_pn]
        # לא נמצא ברשימת החוסרים בפועל - כלומר יש ממנו מספיק מלאי, ולכן
        # הוא זמין כבר היום ולא משפיע על הנתיב הקריטי, גם אם רשום לו
        # ETA עתידי (זה לא הופך אותו לחוסם - יש כבר מספיק בקופה).
        if comp_pn not in _cp_shortage_pns:
            result = (_today_ts, False)
            _cp_eta_cache[comp_pn] = result
            return result
        eta_str = get_first_supply_eta(comp_pn, _cp_inv_cache)
        if eta_str and eta_str != "בדיקה נדרשת":
            try:
                result = (pd.to_datetime(eta_str), False)
                _cp_eta_cache[comp_pn] = result
                return result
            except Exception:
                pass
        result = (_assumed_date, True)
        _cp_eta_cache[comp_pn] = result
        return result

    def _cp_compute(asm_pn, visited):
        if asm_pn in _cp_cache:
            return _cp_cache[asm_pn]
        if asm_pn in visited:
            return None, False, None, None, None, None
        visited = visited | {asm_pn}

        latest_date, is_assumption = None, False
        limiting_type, limiting_ref, limiting_display, limiting_comp_desc = None, None, None, None

        # שיפור ביצועים: משתמשים ב-ASSEMBLY_DIRECT_COMPONENTS שכבר בנוי
        # פעם אחת מראש בטעינת הקובץ (PN, כמות, תיאור, מלאי - לכל רכיבי
        # הגלם הישירים של ההרכבה) - במקום df.iterrows() שהיה סורק את כל
        # שורות הטבלה (מאות שורות) בכל קריאה, לכל אחת מ-26+ ההרכבות. זה
        # בדיוק אותו אינדקס שכבר משמש את חישובי ה-CTB/קיבולת מקסימלית
        # במקומות אחרים באפליקציה, ולכן לא צריך לבנות שום דבר חדש.
        for comp_pn, qty_per, comp_desc, base_stock_check in ASSEMBLY_DIRECT_COMPONENTS.get(asm_pn, []):
            if qty_per <= 0:
                continue
            if base_stock_check >= 9000000:
                continue  # מק"ט של תת-הרכבה - מטופל למטה, לא רכיב גלם אמיתי
            comp_date, comp_assumed = _cp_get_component_eta_date(comp_pn)
            if latest_date is None or comp_date > latest_date:
                latest_date = comp_date
                is_assumption = comp_assumed
                limiting_type, limiting_ref = "component", comp_pn
                limiting_comp_desc = str(comp_desc)
                limiting_display = f"{comp_pn} ({str(comp_desc)[:24]})"

        for child_pn in ASSEMBLY_CHILDREN.get(asm_pn, []):
            child_date, child_assumed, _, _, child_display, _ = _cp_compute(child_pn, visited)
            if child_date is not None and (latest_date is None or child_date > latest_date):
                latest_date = child_date
                is_assumption = child_assumed
                limiting_type, limiting_ref = "subassembly", child_pn
                limiting_comp_desc = None
                limiting_display = f"{child_pn} (תת-הרכבה) ← {child_display}" if child_display else f"{child_pn} (תת-הרכבה)"

        if latest_date is None:
            # אין שום רכיב/תת-הרכבה חוסמים - ההרכבה מוכנה כבר היום מבחינת
            # קלטים (לפני תוספת זמן הבנייה של עצמה, למטה).
            latest_date, is_assumption = _today_ts, False
            limiting_type, limiting_ref, limiting_display, limiting_comp_desc = None, None, "אין רכיבים/תתי-הרכבות שחוסמים", None

        # תיקון: מוסיפים את זמן הבנייה של ההרכבה הזו עצמה (יומיים) מעבר
        # לזמינות המקסימלית של הקלטים שלה - כך שעיכוב מצטבר עם כל רמה
        # נוספת בעץ (הרכבה תחתונה שמעוכבת בגלל רכיב חסר גוררת עיכוב לכל
        # האבות שלה במעלה העץ, ועוד +2 ימים בכל רמה בנוסף).
        latest_date = latest_date + pd.Timedelta(days=_BUILD_DAYS_PER_ASSEMBLY)

        _cp_cache[asm_pn] = (latest_date, is_assumption, limiting_type, limiting_ref, limiting_display, limiting_comp_desc)
        return _cp_cache[asm_pn]

    for _asm in valid_assemblies:
        _cp_compute(_asm, set())

    # שרשרת החסימה בפועל: מתחילים מכל הרכבת-שורש (רמה 0) ויורדים תמיד
    # לכיוון תת-ההרכבה שהוגדרה כ"גורם המגביל" (limiting_type=="subassembly")
    # - השרשרת הזו היא בדיוק "מי אשם" בעיכוב של ראש העץ.
    _cp_roots = [pn for pn, info in ASSEMBLY_BOM_TREE.items() if info.get("parent") is None]

    def _build_critical_chain(root_pn):
        chain = set()
        current = root_pn
        guard = 0
        while current is not None and guard < 200:
            chain.add(current)
            _, _, ltype, lref, _, _ = _cp_cache.get(current, (None, False, None, None, None, None))
            current = lref if ltype == "subassembly" else None
            guard += 1
        return chain

    _cp_critical_set = set()
    for _root in _cp_roots:
        _cp_critical_set |= _build_critical_chain(_root)

    # ---------- טבלת סיכום ----------
    _cp_rows = []
    for _asm in valid_assemblies:
        _date, _assumed, _ltype, _lref, _ldisp, _ = _cp_cache.get(_asm, (None, False, None, None, None, None))
        _cp_rows.append({
            "קוד הרכבה": _asm,
            "תיאור": ASSEMBLY_BOM_TREE.get(_asm, {}).get("desc", ""),
            "רמה בעץ": ASSEMBLY_BOM_TREE.get(_asm, {}).get("level", 0),
            "על נתיב החסימה?": "🔴 כן" if _asm in _cp_critical_set else "לא",
            "תאריך מוכנות (נתיב קריטי)": (_date.strftime("%Y-%m-%d") if _date is not None else "—") + (" (הנחה)" if _assumed else ""),
            "_sort_date": _date if _date is not None else _today_ts,
            "מבוסס על הנחה": "כן (הנחה)" if _assumed else "לא (ETA אמיתי)",
            "הגורם המגביל בשרשרת": _ldisp or "—",
        })
    ...
    with tab_tree:
        st.caption("🔴 = ההרכבה הזו (או שרשרת מתחתיה) קובעת בפועל את תאריך המוכנות של ראש העץ. לחצו על כל שורה כדי לפתוח/לסגור את הרמה הבאה.")

        def _render_cp_node(asm_pn, depth):
            info = ASSEMBLY_BOM_TREE.get(asm_pn, {})
            date, assumed, ltype, lref, _, lcompdesc = _cp_cache.get(asm_pn, (None, False, None, None, None, None))
            is_critical = asm_pn in _cp_critical_set
            date_str = (date.strftime("%Y-%m-%d") if date is not None else "—") + (" (הנחה)" if assumed else "")
            icon = "🔴" if is_critical else "⚪"
            label = f"{icon} {asm_pn} — {info.get('desc', '')}  |  מוכנות: {date_str}"
            children = sorted(ASSEMBLY_CHILDREN.get(asm_pn, []), key=lambda c: 0 if c in _cp_critical_set else 1)

            # תיקון (בעקבות משוב): כשההרכבה הזו היא בדיוק סוף שרשרת החסימה
            # (הגורם המגביל שלה הוא רכיב גלם ממשי, לא תת-הרכבה נוספת) -
            # מציגים את הרכיב הספציפי הזה במפורש, מודגש באדום, בתוך הענף
            # שלו - כדי לדעת בדיוק על איזה רכיב לעבוד כדי לקצר את הנתיב.
            show_blocking_component = is_critical and ltype == "component" and lref

            if children or show_blocking_component:
                with st.expander(label, expanded=(depth == 0 or is_critical)):
                    if show_blocking_component:
                        st.markdown(
                            f"<div style='padding:8px 12px; margin-bottom:8px; background:rgba(239,68,68,0.15); "
                            f"border-right:4px solid {DANGER}; border-radius:6px;'>"
                            f"<b style='color:{DANGER};'>🔴 הרכיב שגורם לנתיב הקריטי בענף הזה:</b> "
                            f"<code>{lref}</code> — {str(lcompdesc)[:60] if lcompdesc else ''}"
                            f"</div>",
                            unsafe_allow_html=True
                        )
                    for c in children:
                        _render_cp_node(c, depth + 1)
            else:
    ...
    with tab_table:
        ...
        if not _top_level.empty:
            ...
            fig_cp = px.bar(
                ...
                hover_data=["תיאור", "הגורם המגביל בשרשרת"],
