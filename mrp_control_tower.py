...
""")
    ...
    st.caption(
        "תאריך המוכנות של כל הרכבה נקבע לפי הרכיב/תת-ההרכבה שמגיע/ה הכי מאוחר בכל השרשרת שמתחתיה - "
        "לא רק הרכיבים הישירים שלה. פריטים בלי ETA בקובץ מניחים הגעה בעוד חודש אחד מהיום (מסומן בבירור כ'הנחה')."
    )
    ...
    _cp_cache = {}  # asm_pn -> (latest_date, is_assumption, limiting_type, limiting_ref, limiting_display)
    ...
    def _cp_compute(asm_pn, visited):
        ...
        if latest_date is None:
            latest_date, is_assumption = _assumed_date, True
            limiting_type, limiting_ref, limiting_display, limiting_comp_desc = None, None, "אין נתוני רכיבים", None
