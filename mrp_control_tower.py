...
if st.sidebar.button("🔄 רענן נתונים (נקה קאש)", help="לוחצים כאן אם עדכנת קובץ ב-GitHub והאפליקציה עדיין מציגה נתונים ישנים - זה מנקה את כל מטמון הנתונים ומכריח טעינה מחדש, בלי לצאת ל-Streamlit Cloud."):
    load_data.clear()
    fetch_all_inventory_records.clear()
    fetch_wip_records.clear()
    fetch_saved_projects.clear()
