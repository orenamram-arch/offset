...
""")
    ...
    # מיפוי pn -> node של השורש שהוא שייך אליו (לכל הרכבה - node ברמת "שורש
    # עצמו" בעץ שחושב לה) - נחוץ לתצוגת עץ/טבלה שמסתמכת על valid_assemblies.
    ...
    for _root, _res in _cp_results.items():
        ...
        for _node in _res["nodes"].values():
            if _node["kind"] == "assembly":
                _by_pn.setdefault(_node["pn"], _node)  # שומרים את המופע הראשון (השורש/העליון ביותר)
