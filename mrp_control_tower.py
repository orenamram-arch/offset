...
<div class="hero-banner">
    <div class="hero-banner-small-label">🚀 MRP CONTROL TOWER</div>
...
""")
    ...
    st.caption("כל מלבן: קוד הרכבה + תיאור. המספר ליד קו החיבור להורה = כמות הקשר (כמה יחידות נדרשות ליחידה אחת של ההורה).")
    ...
    if not _bom_positions:
        ...
        for pn, info in ASSEMBLY_BOM_TREE.items():
            ...
            annotations.append(dict(
                x=x_, y=y_, text=f"<b>{pn}</b><br>{desc}", showarrow=False,
        ...
        fig_tree.update_layout(
            ...
            yaxis=dict(visible=False, range=[-0.8, max_y + 0.8], autorange="reversed"),
            template=PLOTLY_TEMPLATE,
            height=max(420, 150 * (max_y + 1)),
            margin=dict(t=10, b=10, l=10, r=10),
            plot_bgcolor="rgba(0,0,0,0)"
        )
        st.plotly_chart(fig_tree, use_container_width=True)
        st.caption(f"סה\"כ {len(_bom_positions)} הרכבות, {max_y + 1} רמות עומק.")

elif nav_page == "🎯 נתיב קריטי":
    # ==========================================================
    # נתיב קריטי לכל הרכבה, עד לראש העץ
    # ==========================================================
    ...
    # לפריט בלי ETA בכלל (למשל "בדיקה נדרשת") - מניחים 4 חודשים קדימה
    ...
    st.caption(
        ...
        "לא רק הרכיבים הישירים שלה. פריטים בלי ETA בקובץ מניחים הגעה בעוד 4 חודשים מהיום (מסומן בבירור כ'הנחה')."
    )

    _cp_inv_cache = fetch_all_inventory_records(PROJECT_ID)
    _ASSUMED_MONTHS_AHEAD = 4
    ...
    def _cp_compute(asm_pn, visited):
        ...
        latest_date, is_assumption, limiting_desc = None, False, None
        ...
        if asm_pn in df.columns:
            for _, row in df.iterrows():
                ...
                if latest_date is None or comp_date > latest_date:
                    ...
                    comp_desc = str(row[DESC_COL])[:24]
                    limiting_desc = f"{comp_pn} ({comp_desc})"

        for child_pn in ASSEMBLY_CHILDREN.get(asm_pn, []):
            child_date, child_assumed, child_limiting = _cp_compute(child_pn, visited)
            if child_date is not None and (latest_date is None or child_date > latest_date):
                ...
                limiting_desc = f"{child_pn} (תת-הרכבה) ← {child_limiting}" if child_limiting else f"{child_pn} (תת-הרכבה)"

        if latest_date is None:
            latest_date, is_assumption, limiting_desc = _assumed_date, True, "אין נתוני רכיבים"

        _cp_cache[asm_pn] = (latest_date, is_assumption, limiting_desc)
        return _cp_cache[asm_pn]

    _cp_rows = []
    for _asm in valid_assemblies:
        _date, _assumed, _limiting = _cp_compute(_asm, set())
        _cp_rows.append({
            "קוד הרכבה": _asm,
            "תיאור": ASSEMBLY_BOM_TREE.get(_asm, {}).get("desc", ""),
            "רמה בעץ": ASSEMBLY_BOM_TREE.get(_asm, {}).get("level", 0),
            "תאריך מוכנות (נתיב קריטי)": _date.strftime("%Y-%m-%d") + (" (הנחה)" if _assumed else ""),
            "_sort_date": _date,
            "מבוסס על הנחה": "כן (הנחה)" if _assumed else "לא (ETA אמיתי)",
            "הגורם המגביל בשרשרת": _limiting or "—",
        })
    ...
    def _cp_highlight(val):
        if isinstance(val, str) and "הנחה" in val:
            return f"background-color: {WARNING}; color: white; font-weight: 700;"
        return ""

    st.dataframe(
        _cp_df.style.map(_cp_highlight, subset=["תאריך מוכנות (נתיב קריטי)", "מבוסס על הנחה"]),
