...
_manual_eta_override = st.session_state.get(f"_manual_block_{PROJECT_ID}_ETA")
...
_manual_plan_override = st.session_state.get(f"_manual_block_{PROJECT_ID}_PLAN")
    ...
    with st.expander(
        ...
        for _block_name, _info in _uncertain_blocks.items():
            ...
            with _oc3:
                st.write("")
                st.write("")
                if st.button("✅ החל תיקון", key=f"_apply_manual_{_block_name}"):
                    _override_key = "ETA" if "ETA" in _block_name else "PLAN"
                    st.session_state[f"_manual_block_{PROJECT_ID}_{_override_key}"] = (int(_manual_start), int(_manual_end))
                    st.success(f"נשמר עבור {_block_name} - טוען מחדש...")
                    st.rerun()
