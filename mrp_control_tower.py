...
""")
    ...
    with tab_priority:
        st.caption(
            ...
            "הופך להיות זה שקובע את התאריך, וכן הלאה. ⚠️ זהו קירוב על בסיס מצב הנתונים הנוכחי - אינו מדמה "
            "מחדש את כל העץ אחרי כל הקדמה היפותטית, אבל נותן סדר עבודה ברור למי לפנות קודם."
        )
        if not _priority_rows_by_root:
            ...
            for _root in sorted(_priority_rows_by_root.keys()):
                ...
                with st.expander(f"🔗 {_root} — {_root_desc}  |  יעד נוכחי: {_root_target}  |  {len(_pdf)} רכיבים חוסמים", expanded=True):
                    st.dataframe(
                        _pdf.style.map(_priority_highlight, subset=["על הנתיב הקריטי?"]),
