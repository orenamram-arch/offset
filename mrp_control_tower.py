...
def fetch_saved_projects():
    ...
    try:
        ...
        # לא בולעים את השגיאה בשקט - אם הטעינה נכשלת (טבלה חסרה, בעיית הרשאות,
        # ניתוק זמני וכו') חשוב שתדע שזו שגיאת חיבור, ולא שהפרויקטים "נמחקו"
        # בפועל. הכישלון גם נשמר ל-90 שניות כדי לא להציף בהודעות על כל רענון.
        _last_fail = st.session_state.get("_saved_projects_fetch_error_ts", 0)
        if time.time() - _last_fail > 90:
            st.session_state["_saved_projects_fetch_error_ts"] = time.time()
            st.warning(
                f"⚠️ לא הצלחתי לטעון פרויקטים שמורים מהענן (הפרויקטים הקבועים בקוד עדיין זמינים). "
                f"זו כנראה שגיאת חיבור זמנית או הרשאות ל-Supabase, לא מחיקה בפועל - נסה 'רענן נתונים' "
                f"בעוד רגע. פירוט: {e}"
            )
        return {}
...
def _apply_cache_bust(url):
    """מוסיף פרמטר-שאילתה ל-URL שמבוסס על טוקן ריענון ב-session (מתעדכן רק
    בלחיצה על '🔄 רענן נתונים') - כדי לעקוף קאש CDN של GitHub (raw.githubusercontent.com
    שומר עותקים ישנים של קבצים לכמה דקות, גם אחרי שהקובץ עודכן ב-main ואחרי שקאש
    Streamlit עצמו נוקה). לא משתנה בכל ריצה רגילה, כדי לא לפגוע בביצועים/קאש הרגיל -
    רק כשהמשתמש מבקש רענון מפורש."""
    if not url:
        return url
    token = st.session_state.get("_cache_bust_token", 0)
    if not token:
        return url
    sep = "&" if "?" in url else "?"
    return f"{url}{sep}_cb={token}"
...
def render_welcome_screen():
    ...
    (function() {
        ...
        // תיקון: מזג אוויר קבוע לישראל (תל אביב, מרכז הארץ) - במקום
        // זיהוי מיקום דינמי של המשתמש (בקשת הרשאת מיקום/IP הוסרה).
        fetchWeather(32.0853, 34.7818, 'ישראל (תל אביב)');
    })();
    </script>
    """, height=640, scrolling=False)

# מיכל בעמוד הראשי שבו יוצג מסך ה-WELCOME אם עדיין לא נבחר פרויקט -
# נוצר כאן, לפני חלונית בחירת הפרויקט למטה, כדי שהתוכן יוצג במקום
# הנכון (מוקדם בעמוד) גם שהתנאי בעצמו מתגלה בתוך הפופאובר.
_main_welcome_slot = st.empty()

st.markdown('<div class="nav-bar-label">📁 פרויקט פעיל</div>', unsafe_allow_html=True)
# תיקון (בעקבות משוב): כפתור הפתיחה הציג טקסט קבוע תמיד ("בחר/החלף פרויקט"),
# גם אחרי שכבר נבחר פרויקט - כך שלא הייתה שום דרך לדעת מבחוץ שהבחירה נקלטה
# בלי לפתוח את הפופאובר מחדש ולהסתכל פנימה (הרגיש "תקוע"/חסר תגובה, למרות
# שהאפליקציה בפועל עבדה). עכשיו תווית הכפתור עצמה מציגה את הפרויקט הפעיל
# בפועל - כך שאפילו כשהפופאובר סגור רואים מיד שהבחירה נקלטה.
_placeholder = "— בחר פרויקט —"
_cur_proj_for_label = st.session_state.get("selected_project_name")
_project_popover_label = (
    f"📁 פרויקט פעיל: {_cur_proj_for_label}"
    if _cur_proj_for_label and _cur_proj_for_label != _placeholder
    else "📁 בחר פרויקט..."
)
_project_popover = st.popover(_project_popover_label, use_container_width=True) if hasattr(st, "popover") else st.expander(_project_popover_label, expanded=True)
with _project_popover:
    st.markdown("### 📁 פרויקט פעיל")
    _placeholder = "— בחר פרויקט —"
    _project_names = [_placeholder] + list(AVAILABLE_PROJECTS.keys()) + ["🔗 קישור מותאם אישית..."]
    # אין טעינה אוטומטית של אף פרויקט קבוע (כולל "אנטנה") - תמיד מתחילים
    # מ"בחר פרויקט" הריק, גם אם נשאר ?project= שמור בכתובת מביקור קודם.
    # זה מונע טעינה בשוגג של פרויקט ישן/לא-רצוי (ולכן גם קריאת ערכים ישנים
    # מה-DB שמקושרים אליו) רק כי הוא נטען בעבר או כי הוא ראשון ברשימה.
    _selected_project = st.selectbox("בחר פרויקט", _project_names, index=0, key="selected_project_name")
    # משוב חזותי מיידי - כדי שברור שהבחירה נקלטה, גם אם הפופאובר עצמו
    # נשאר פתוח (זו התנהגות ברירת מחדל תקינה של Streamlit - לא נסגר לבד
    # אחרי בחירה, ולכן חשוב משוב אחר שמראה שמשהו קרה בפועל).
    if _selected_project != _placeholder:
        st.success(f"✅ נטען: {_selected_project}")
...
# פורץ קאש CDN של GitHub אוטומטית אחרי לחיצה על "רענן נתונים" (ראה
# _apply_cache_bust למעלה) - בלי זה, URL זהה יכול להמשיך להגיש עותק ישן
# מה-CDN גם אחרי שהקובץ עודכן ב-GitHub וגם אחרי שקאש Streamlit נוקה.
ACTIVE_PROJECT_URL = _apply_cache_bust(ACTIVE_PROJECT_URL)
...
/* ==========================================================
   שיפור: הסתרת אייקונים/קישורים שמובילים החוצה מהאפליקציה (GitHub,
   Streamlit, כפתור Deploy, סרגל הכלים העליון) - בלי לפגוע בחצי
   (הכפתור) לפתיחת הסיידבר במובייל.
   ==========================================================
   תיקון קריטי (בעקבות דיווח בפועל): הגרסה הקודמת הסתירה את כל ה-
   header (visibility:hidden; height:0) - אבל כפתור פתיחת הסיידבר
   כשהוא מכווץ (במיוחד במובייל, שבו הסיידבר מתחיל מכווץ כברירת מחדל)
   יושב פיזית בתוך אותו אזור header, ולכן נעלם יחד איתו. זה בדיוק מה
   שגרם לכך שאי אפשר היה לחזור ל"וילון" המסננים במובייל אחרי סגירתו.
   התיקון: לא מסתירים את ה-header כולו יותר - רק את הכפתורים/קישורים
   הספציפיים שרוצים להעלים (Toolbar עם Share/GitHub/Deploy), ומוודאים
   באופן מפורש שכפתור פתיחת הסיידבר תמיד נשאר גלוי ולחיץ.
   ========================================================== */
[data-testid="stToolbar"] {{ visibility: hidden; }}
[data-testid="stDecoration"] {{ display: none !important; }}
.stAppDeployButton {{ display: none !important; }}
a[href*="github.com"], a[href*="streamlit.io"] {{ display: none !important; pointer-events: none !important; }}

/* רשת ביטחון מפורשת: לוודא שכפתור פתיחת/סגירת הסיידבר (החץ שמופיע
   כשהסיידבר מכווץ, קריטי במובייל) תמיד נשאר גלוי ולחיץ, לא משנה אילו
   כללים אחרים למעלה בטעות היו עלולים להשפיע עליו.
   תיקון נוסף (בעקבות דיווח חוזר): הבעיה לא הייתה רק "מוסתר" - פס
   תקציר המנהלים (exec-summary-strip) הוא position:sticky עם
   z-index:999, מודבק לראש האזור הראשי. גם כשהחץ לפתיחת הסיידבר לא
   היה "מוסתר" טכנית, פס התקציר-הדביק היה עלול לשבת מעליו ויזואלית
   ולחסום את הלחיצה. עכשיו נותנים לחץ z-index גבוה במפורש יותר מהפס
   (1000 > 999), עם position:fixed מוצהר, כדי שהוא תמיד יישאר מעל כל
   תוכן אחר בעמוד ותמיד לחיץ. */
...
/* תיקון: בפופאובר עם הרבה תוכן (מסננים/הגדרות) לא היה אפשר לגלול
   למטה כדי לראות את כל האפשרויות במובייל - הפופאובר לא הגביל את
   הגובה שלו וגלש/נחתך מחוץ למסך בלי scroll פנימי. עכשיו הוא מוגבל
   בגובה מקסימלי עם גלילה פנימית משלו, כך שתמיד רואים הכל, גם במסך קטן. */
[data-testid="stPopoverBody"] {{
    max-height: 75vh !important;
    overflow-y: auto !important;
}}
/* תיקון: מיכל כל תרשים Plotly (כולל עץ ה-BOM הרחב) מקבל גלילה עצמאית
   (אופקית ואנכית) בתוך עצמו, במקום לגלוש/להיחתך מחוץ למסך במובייל.
   ביחד עם staticPlot=True על תרשים העץ (מוגדר בקוד עצמו), זה גם מונע
   מגע/גרירה בתוך התרשים "ללכוד" את הגלילה של העמוד כולו - מגע בתוך
   התרשים גולל רק אותו, וגלילה מחוץ לו גוללת את העמוד כרגיל וחוזרת
   בקלות לתפריטי הניווט למעלה. */
...
# ==========================================================
# תיקון קריטי: כפתור מותאם-אישית לפתיחת הסיידבר במובייל
# ==========================================================
# שני ניסיונות CSS קודמים לא פתרו את הבעיה - ייתכן שהטסט-איידי
# (data-testid) שהשתמשתי בו לא תואם בדיוק את גרסת ה-Streamlit הרצה
# כאן. במקום להמשיך "לנחש" סלקטור CSS, זהו כפתור עצמאי משלי (☰),
# תמיד גלוי, קבוע בפינה השמאלית-עליונה, עם z-index גבוה מאוד - הוא
# חלק אמיתי מה-DOM הראשי (לא בתוך iframe), ולכן ה-onclick המוטמע בו
# (attribute, לא <script> נפרד - כי תגיות <script> שמוזרקות דרך
# innerHTML לא רצות בדפדפן) מנסה למצוא ולהפעיל את כפתור הסיידבר
# המקורי של Streamlit לפי כמה גרסאות אפשריות של השם הפנימי שלו -
# וכך זה לא תלוי בניחוש נכון של גרסה ספציפית אחת.
st.markdown("""
<style>
...
#custom-sidebar-toggle-btn:active { transform: scale(0.94); }
</style>
<button id="custom-sidebar-toggle-btn" title="פתח/סגור תפריט מסננים" onclick="
    var doc = document;
    ...
    if (btn) { btn.click(); } else { alert('לא נמצא כפתור סיידבר מובנה - נסה לגלול את המסך למעלה, או לרענן את הדף.'); }
">☰</button>
""", unsafe_allow_html=True)

st.markdown(f"""
<div class="hero-banner">
    <div class="hero-banner-small-label">🚀 MRP CONTROL TOWER &nbsp;|&nbsp; פרויקט: {PROJECT_ID}</div>
    <h1>{st.session_state.get("main_nav_page", "📈 Executive Dashboard")}</h1>
</div>
""", unsafe_allow_html=True)
...
def save_wip_record(assembly_pn, wip_qty):
    ...
    try:
        ...
        st.error(f"❌ שגיאה בשמירת WIP ל-Supabase (לא נשמר בפועל): {e}")
        return False
...
def delete_wip_record(assembly_pn):
    try:
        ...
        st.error(f"❌ שגיאה במחיקת WIP מ-Supabase (לא נמחק בפועל): {e}")
        return False
...
def save_inventory_record(pn, added_stock, eta, status, supplier, comment, updated_by, webhook_url="", item_type=None, unit_price=None):
    ...
    try:
        ...
        st.error(f"❌ שגיאה בשמירה ל-Supabase (העדכון לא נשמר בפועל): {e}")
        save_ok = False

    if webhook_url and save_ok:
...
def delete_inventory_record(pn):
    try:
        ...
        st.error(f"❌ שגיאה במחיקה מ-Supabase (לא נמחק בפועל): {e}")
        return False
...
@st.cache_data(ttl=60)
def fetch_block_overrides(project_id):
    """תיקונים ידניים לזיהוי בלוקי-עמודות (ETA / PROJECT PLAN וכו') שהמשתמש
    ציין בעבר לפרויקט הזה - נשמרים לצמיתות (לא רק ל-session), כדי שלא יהיה
    צורך לתקן שוב בכל כניסה. מבנה טבלה גנרי (project_id, block_type, start_col,
    end_col) - אם בעתיד יתווספו סוגי בלוקים נוספים, זו אותה טבלה בדיוק, בלי
    שינוי סכמה."""
    try:
        response = supabase.table("mrp_block_overrides").select("*").eq("project_id", project_id).execute()
        return {row["block_type"]: (row["start_col"], row["end_col"]) for row in (response.data or [])}
    except Exception:
        return {}

def save_block_override(project_id, block_type, start_col, end_col):
    try:
        supabase.table("mrp_block_overrides").upsert(
            {"project_id": project_id, "block_type": block_type, "start_col": int(start_col), "end_col": int(end_col)},
            on_conflict="project_id,block_type"
        ).execute()
        fetch_block_overrides.clear()
        return True, None
    except Exception as e:
        return False, str(e)

def delete_block_override(project_id, block_type):
    try:
        supabase.table("mrp_block_overrides").delete().eq("project_id", project_id).eq("block_type", block_type).execute()
        fetch_block_overrides.clear()
        return True, None
    except Exception as e:
        return False, str(e)
...
# ==========================================================
_MATRIX_HEADER_ANCHORS = ("#", "SL NO", "SL. NO", "SL NO.", "S/N", "S.N.")

def _find_matrix_header_row(raw_df):
    """מאתר את שורת הכותרת הראשית של הגיליון: עמודת מונה (מספר סידורי - יכולה
    להופיע כ-'#' או כ-'SL No' / 'S/N' בקבצים אחרים) ואיפשהו בסמוך לה (עד 5
    עמודות קדימה - כדי לסבול עמודה ריקה ביניהן, כמו שקורה בחלק מהקבצים) עמודת
    'PN_ID'. אם לא נמצא עוגן מונה כלל, נופלים לחיפוש 'PN_ID' לבד בעשר העמודות
    הראשונות - כדי לא להיכשל על פורמט עוגן חדש שעוד לא נתקלנו בו."""
    for r in range(raw_df.shape[0]):
        anchor_cols = [c for c in range(min(10, raw_df.shape[1]))
                       if isinstance(raw_df.iat[r, c], str)
                       and raw_df.iat[r, c].strip().upper() in _MATRIX_HEADER_ANCHORS]
        if not anchor_cols:
            continue
        anchor_col = anchor_cols[0]
        for c2 in range(anchor_col, min(anchor_col + 6, raw_df.shape[1])):
            v = raw_df.iat[r, c2]
            if isinstance(v, str) and v.strip().upper() == "PN_ID":
                return r
    # Fallback: אין עוגן מונה מוכר, אבל אולי יש עדיין PN_ID בעשר העמודות
    # הראשונות של השורה - עדיף מנפילה ל-header_row קשיח שעלול להיות שגוי לגמרי.
    for r in range(raw_df.shape[0]):
...
def _find_supply_range(raw_df, named_cols, end_idx_exclusive):
    """מזהה את בלוק ה-ETA/אספקה: קודם מנסה עוגן טקסטואלי אמיתי - תא עם הטקסט
    המדויק 'ETA' בגיליון (נבדק ואומת שקיים בדיוק בעמודה הראשונה של הבלוק הנכון
    בכל קבצי הפרויקטים שנבדקו - אותה שיטת עוגן-טקסט שכבר קיימת לזיהוי DEMAND).
    זה מדויק ואמין הרבה יותר מהיוריסטיקת-מיקום גרידא, כי הוא לא תלוי בכך שבלוק
    האספקה במקרה יושב הכי קרוב לעץ ה-BOM - מה שיכול להשתבש אם בלוק אחר (כמו
    PROJECT PLAN) יושב קרוב יותר בפריסה מסוימת. אם לא נמצאה תווית 'ETA' בכלל
    (קובץ ישן/שונה בלי התווית הזו) - נופלים בבטחה להיוריסטיקת המיקום הקודמת,
    כדי לא לשבור תאימות לאחור עם קבצים שכבר עובדים נכון היום."""
    block = _find_label_anchored_date_block(raw_df, named_cols, ["ETA"])
    if block:
        return block[0]
    return _longest_date_run_ending_before(named_cols, end_idx_exclusive)

def _find_label_anchored_date_block(raw_df, named_cols, labels):
    """מחפש כל אחת מהתוויות ברשימת labels (טקסט מדויק, לא תלוי-רישיות) בגיליון
    הגולמי, ומחזיר את בלוק התאריכים הרציף שמתחיל בדיוק בעמודה שבה נמצאה
    התווית (רק אם היא אכן עמודת-תאריך בכותרת בפועל). מחזיר גם את מיקום התווית
    ואת שם התווית שנמצאה (לצורך תצוגה למשתמש/אבחון) - (block, hit_position, label).
    מחזיר None אם אף תווית ברשימה לא נמצאה, או שהעמודה שמתחתיה אינה עמודת-תאריך."""
    for label in labels:
        hit = _find_text_cell(raw_df, label)
        if hit is None:
            continue
        _, col = hit
        if col < len(named_cols) and _is_date_col(named_cols[col]):
            end = col
            while end < len(named_cols) and _is_date_col(named_cols[end]):
                end += 1
            return (col, end), hit, label
    return None
...
def _find_column_by_candidates(named_cols, candidates):
    ...
    אם כמה עמודות שונות מתאימות לאותו שם מנורמל (קורה בקבצים עם בלוק עץ BOM
    נפרד שמשתמש שוב בשמות כמו 'DESC'/'description') - לוקחים תמיד את המופע
    הראשון (השמאלי ביותר), כי זו כמעט תמיד עמודת המטא-דאטה האמיתית לכל שורה,
    לא הכפילות המאוחרת יותר."""
    norm = {}
    for c in named_cols:
        key = str(c).strip().upper()
        if key not in norm:  # שומרים רק את המופע הראשון, לא דורסים בהמשך
            norm[key] = c
...
# ==========================================================
# מבנה נתונים מוכן-מראש: לכל הרכבה, רשימת הרכיבים הישירים שלה (מתוך מטריצת
# ה-where-used) - מחושב פעם אחת בטעינת הפרויקט, במקום לסרוק את כל הטבלה
# (df.iterrows(), שהיא איטית מאוד) בכל קריאה רקורסיבית לחישובי CTB/קיבולת
# מקסימלית. זה חוסך המון זמן ריצה במיוחד בעצים עמוקים/רחבים, כי הפונקציות
# האלה נקראות שוב ושוב (רקורסיבית, ולפעמים כמה פעמים לאותה הרכבה דרך ענפים
# משותפים) - סריקת הטבלה המלאה בכל פעם הופכת לאיטית בצורה דרמטית.
# ==========================================================
ASSEMBLY_DIRECT_COMPONENTS = {}
if valid_assemblies:
    _pn_series = df[PN_COL].astype(str).str.strip()
    _desc_series = df[DESC_COL] if DESC_COL in df.columns else pd.Series([""] * len(df), index=df.index)
    _stock_series = df[STOCK_COL].apply(lambda v: safe_num(v)) if STOCK_COL in df.columns else pd.Series([0.0] * len(df), index=df.index)
    for _asm_col in valid_assemblies:
        if _asm_col not in df.columns:
            continue
        _qty_series = df[_asm_col].apply(lambda v: safe_num(v))
        _mask = _qty_series > 0
        if not _mask.any():
            continue
        ASSEMBLY_DIRECT_COMPONENTS[_asm_col] = list(zip(
            _pn_series[_mask].tolist(),
            _qty_series[_mask].tolist(),
            _desc_series[_mask].astype(str).tolist(),
            _stock_series[_mask].tolist()
        ))
else:
    _pn_series = df[PN_COL].astype(str).str.strip()

# מיפוי הפוך: לכל רכיב (PN), באילו הרכבות הוא משתתף ובאיזו כמות - נחוץ כדי
# לחשב "כמה מהרכיב הזה כבר תפוס ב-WIP" בלי לסרוק את כל עמודות ההרכבות בכל
# פעם (ראה שימוש ב-get_component_available_by_month למטה).
COMPONENT_TO_ASSEMBLIES = {}
for _asm_col, _comps in ASSEMBLY_DIRECT_COMPONENTS.items():
    for _comp_pn, _qty_per, _c_desc, _c_stock in _comps:
        COMPONENT_TO_ASSEMBLIES.setdefault(_comp_pn, []).append((_asm_col, _qty_per))

# מיפוי PN -> אינדקס שורה ב-df, כדי להימנע מסינון+המרת-מחרוזות על כל הטבלה
# בכל קריאה בודדת (זה היה צוואר הבקבוק המרכזי בזמן ריצה של חישובי ה-CTB/
# קיבולת מקסימלית, שקוראים לפונקציה הזו הרבה פעמים בצורה רקורסיבית).
PN_ROW_INDEX = {}
for _idx, _pn_val in _pn_series.items():
    if _pn_val not in PN_ROW_INDEX:
        PN_ROW_INDEX[_pn_val] = _idx

# מיפוי PN -> אינדקס שורה ב-df_raw (לפי עמודת PN_ID הגולמית) - נחוץ
# ל-get_base_mrp_eta_and_qty/get_cumulative_incoming_supply, שנקראות המון
# פעמים בחישובי CTB/קיבולת מקסימלית/נתיב קריטי, ובעבר סיננו את כל df_raw
# (כולל המרת מחרוזות) בכל קריאה בודדת.
#
# תיקון קריטי (בעקבות משוב): לפני התיקון המיקום היה מקובע ("עמודה 1") -
# אבל זה נכון רק בקבצים שבהם PN_ID באמת יושב בעמודה הגולמית הראשונה אחרי
# ה-SL No/#. בקבצים אחרים (למשל HPR.xlsx, שם הסדר הוא SL No, Internal Ref,
# PN_ID - כלומר PN_ID בעמודה 2, לא 1) זה גרם למיפוי שגוי לגמרי - נבנה
# בפועל ממחרוזות "Internal Ref" (למשל "LT01-299") במקום PN_ID אמיתי
# ("TS0303W3S"), כך שכל חיפוש לפי PN_ID אמיתי נכשל תמיד ("בדיקה נדרשת"),
# גם כשיש לרכיב ETA אמיתי מלא בקובץ. עכשיו נגזר דינמית מ-PN_COL (שכבר
# מזוהה נכון לפי שם העמודה "PN_ID", לא לפי מיקום קבוע) - מיקום העמודה
# ב-df ותואם בדיוק לאותו מיקום ב-df_raw (סדר העמודות זהה בשני הטבלאות).
_pn_col_pos_in_raw = df.columns.get_loc(PN_COL)
_df_raw_pnid_series = df_raw.iloc[:, _pn_col_pos_in_raw].astype(str).str.strip()
DF_RAW_PN_ROW_INDEX = {}
for _idx, _pn_val in _df_raw_pnid_series.items():
    if _pn_val not in DF_RAW_PN_ROW_INDEX:
        DF_RAW_PN_ROW_INDEX[_pn_val] = _idx

# ==========================================================
# זיהוי דינמי של גבולות בלוקי החודשים (תוכנית עבודה / DEMAND / אספקה-ETA) -
# כל בלוק מזוהה קודם לפי תווית טקסט מפורשת בגיליון (הכי אמין - "ETA",
# "DEMAND", "PROJECT PLAN"/"WORK PLAN"), ורק אם לא נמצאה תווית מתאימה נופלים
# בבטחה להיוריסטיקת מיקום. לכל בלוק נשמרת גם רמת הביטחון בזיהוי - אם משהו
# לא זוהה בביטחון גבוה, מוצגת למשתמש התראה עם אפשרות לתקן ידנית (ראה
# BLOCK_DETECTION_INFO והתצוגה בהמשך), במקום לנחש בשקט או להפיל את הטעינה.
# ==========================================================
BLOCK_DETECTION_INFO = {}
_saved_block_overrides = fetch_block_overrides(PROJECT_ID)
...
_manual_eta_override = _saved_block_overrides.get("ETA")
if _manual_eta_override:
    _supply_range = _manual_eta_override
    BLOCK_DETECTION_INFO["ETA / אספקה"] = {"confidence": "manual", "range": _supply_range}
else:
    _eta_label_result = _find_label_anchored_date_block(df_raw, df.columns, ["ETA"]) if _dlp else None
    if _eta_label_result:
        _supply_range = _eta_label_result[0]
        BLOCK_DETECTION_INFO["ETA / אספקה"] = {"confidence": "label", "range": _supply_range}
    else:
        _supply_range = _longest_date_run_ending_before(df.columns, _dlp["desc"]) if _dlp else None
        BLOCK_DETECTION_INFO["ETA / אספקה"] = {"confidence": "heuristic" if _supply_range else "missing", "range": _supply_range}

_manual_plan_override = _saved_block_overrides.get("PLAN")
_plan_label_result = None if _manual_plan_override else _find_label_anchored_date_block(df_raw, df.columns, ["PROJECT PLAN", "WORK PLAN"])

if _manual_plan_override:
    MONTH_COLS_START_IDX, _plan_end_idx = _manual_plan_override
    MONTH_COLS = df.columns[MONTH_COLS_START_IDX:_plan_end_idx].tolist()
    BLOCK_DETECTION_INFO["PROJECT PLAN / תוכנית עבודה"] = {"confidence": "manual", "range": (MONTH_COLS_START_IDX, _plan_end_idx)}
elif _plan_label_result:
    (_p_start, _p_end), _p_hit, _p_label = _plan_label_result
    MONTH_COLS = df.columns[_p_start:_p_end].tolist()
    MONTH_COLS_START_IDX = _p_start
    BLOCK_DETECTION_INFO["PROJECT PLAN / תוכנית עבודה"] = {"confidence": "label", "range": (_p_start, _p_end)}
elif _plan_start_idx is not None and _demand_start_idx is not None and _demand_start_idx > _plan_start_idx:
    MONTH_COLS = df.columns[_plan_start_idx:_demand_start_idx].tolist()
    MONTH_COLS_START_IDX = _plan_start_idx
    BLOCK_DETECTION_INFO["PROJECT PLAN / תוכנית עבודה"] = {"confidence": "heuristic", "range": (_plan_start_idx, _demand_start_idx)}
elif _plan_start_idx is not None:
    # אין תווית "PROJECT PLAN"/"WORK PLAN" ואין תווית DEMAND - מחפשים את הרצף
    # הרציף הראשון והמשמעותי (לפחות 3 חודשים ברצף, כדי לא להיתפס לתא-תאריך
    # בודד במקרה) של עמודות-תאריך, איפשהו אחרי סוף עץ ה-BOM/המטריצה - לא
    # בהכרח ממש בעמודה הבאה (בקבצים מסוימים יש פער של עשרות עמודות של
    # מטא-דאטה נוספת לפני שבלוק החודשים בפועל מתחיל).
    _min_run = 3
    _search_i = _plan_start_idx
    _found = []
    while _search_i < len(df.columns):
        if _is_date_col(df.columns[_search_i]):
            _run = []
            _j = _search_i
            while _j < len(df.columns) and _is_date_col(df.columns[_j]):
                _run.append(_j)
                _j += 1
            if len(_run) >= _min_run:
                _found = _run
                break
            _search_i = _j + 1
        else:
    ...
    # קריטי: הרצף האמיתי יכול להתחיל הרבה עמודות אחרי _plan_start_idx (למשל
    # פער של עשרות עמודות מטא-דאטה) - אם ממשיכים להשתמש ב-_plan_start_idx
    # כעוגן להמרת "אינדקס בתוך MONTH_COLS" לכל מקום אחר בקוד (כמו קריאת הכמות
    # מ-df_raw בבניית תוכנית ההרכבה), נקבל חוסר-יישור מוחלט - קוראים כמות
    # מעמודה שגויה לגמרי. לכן שומרים את מיקום ההתחלה *האמיתי* בנפרד.
    MONTH_COLS_START_IDX = _found[0] if _found else _plan_start_idx
    BLOCK_DETECTION_INFO["PROJECT PLAN / תוכנית עבודה"] = {
        "confidence": "heuristic" if _found else "missing",
        "range": (MONTH_COLS_START_IDX, _found[-1] + 1) if _found else None
    }
else:
    # נפילה בטוחה למבנה הידוע היסטורית
    MONTH_COLS = df.columns[108:132].tolist() if len(df.columns) > 132 else []
    MONTH_COLS_START_IDX = 108
    BLOCK_DETECTION_INFO["PROJECT PLAN / תוכנית עבודה"] = {"confidence": "missing", "range": (108, 132)}

# אם משהו לא זוהה בביטחון גבוה (לא נמצאה תווית טקסט מפורשת) - לא מנחשים בשקט
# וגם לא מפילים את הטעינה. במקום זה, מציגים התראה עם אפשרות לתקן ידנית את
# טווח העמודות. האפליקציה ממשיכה לעבוד עם הניחוש הטוב ביותר שלה עד אז.
_uncertain_blocks = {k: v for k, v in BLOCK_DETECTION_INFO.items() if v["confidence"] in ("heuristic", "missing")}
if _uncertain_blocks:
    with st.expander(
        f"⚠️ זיהוי אוטומטי לא לגמרי ודאי עבור {len(_uncertain_blocks)} בלוק/ים בקובץ הזה - "
        f"לחץ להצגה ותיקון ידני (לא חובה, האפליקציה ממשיכה עם הניחוש הטוב ביותר שלה)",
        expanded=False
    ):
        st.caption(
            "המערכת מחפשת תווית טקסט מפורשת (כמו 'ETA', 'DEMAND', 'PROJECT PLAN'/'WORK PLAN') "
            "ממש מעל בלוק התאריכים הרלוונטי, כדי לזהות אותו בוודאות. עבור הבלוקים הבאים לא "
            "נמצאה תווית כזו בקובץ הזה, אז נעשה שימוש בניחוש לפי מיקום. אם נתוני ETA/תוכנית "
            "עבודה בטאבים הרלוונטיים נראים שגויים, אפשר לציין כאן ידנית את טווח העמודות הנכון "
            "(מספרי עמודה, החל מ-0) - התיקון יישמר לצמיתות לפרויקט הזה, ולא יהיה צורך לחזור עליו:"
        )
        for _block_name, _info in _uncertain_blocks.items():
            _range_txt = f" (ניחוש נוכחי: עמודות {_info['range'][0]}-{_info['range'][1]-1})" if _info.get("range") else " (לא זוהה כלל)"
            st.markdown(f"**{_block_name}**{_range_txt}")
            _default_start = _info["range"][0] if _info.get("range") else 0
            _default_end = _info["range"][1] if _info.get("range") else 1
            _oc1, _oc2, _oc3 = st.columns([2, 2, 1])
            with _oc1:
                _manual_start = st.number_input(
                    f"עמודת התחלה - {_block_name}", min_value=0, max_value=max(0, len(df.columns) - 1),
                    value=min(_default_start, max(0, len(df.columns) - 1)), key=f"_manual_start_{_block_name}"
                )
            with _oc2:
                _manual_end = st.number_input(
                    f"עמודת סיום (לא כולל) - {_block_name}", min_value=1, max_value=len(df.columns),
                    value=min(_default_end, len(df.columns)), key=f"_manual_end_{_block_name}"
                )
            with _oc3:
                st.write("")
                st.write("")
                if st.button("✅ החל תיקון (נשמר לצמיתות)", key=f"_apply_manual_{_block_name}"):
                    _override_key = "ETA" if "ETA" in _block_name else "PLAN"
                    _ok, _err = save_block_override(PROJECT_ID, _override_key, int(_manual_start), int(_manual_end))
                    if _ok:
                        st.success(f"נשמר לצמיתות עבור '{PROJECT_ID}' - טוען מחדש...")
                        st.rerun()
                    else:
                        st.error(f"שגיאה בשמירה: {_err}")

# תיקונים ידניים שכבר נשמרו לפרויקט הזה - מוצגים תמיד (גם אם אין כרגע בלוקים
# לא-ודאים) עם אפשרות לבטל ולחזור לזיהוי האוטומטי, למקרה שהתיקון עצמו שגוי
# או שכבר לא נחוץ (למשל אחרי שהקובץ עודכן ועכשיו יש בו תווית מפורשת).
if _saved_block_overrides:
    with st.expander(f"🔧 תיקוני עמודות ידניים שמורים לפרויקט '{PROJECT_ID}' ({len(_saved_block_overrides)})", expanded=False):
        for _bt, (_s, _e) in _saved_block_overrides.items():
            _bt_label = "ETA / אספקה" if _bt == "ETA" else "PROJECT PLAN / תוכנית עבודה"
            _rc1, _rc2 = st.columns([3, 1])
            with _rc1:
                st.markdown(f"**{_bt_label}**: עמודות {_s}-{_e-1}")
            with _rc2:
                if st.button("🗑️ בטל תיקון", key=f"_reset_manual_{_bt}"):
                    _ok, _err = delete_block_override(PROJECT_ID, _bt)
                    if _ok:
                        st.success("בוטל - חוזר לזיהוי אוטומטי...")
                        st.rerun()
                    else:
                        st.error(f"שגיאה בביטול: {_err}")
...
if st.session_state.get("_custom_assembly_plan_project") != PROJECT_ID:
    ...
    if not cloud_plan.empty:
        ...
        plan_col_start = MONTH_COLS_START_IDX if MONTH_COLS_START_IDX is not None else 108
        # לזיהוי השורה של כל פריט: אם יש בלוק DESC/LEVEL/PN נפרד (_dlp) - חובה
        # להשתמש בעמודת ה-PN *שלו* (_dlp["pn"]), כי כמויות תוכנית העבודה בפועל
        # רשומות שם, בשורות רשימת-העץ - לא בעמודת ה-PN_ID הכללית (PN_COL),
        # שמייצגת רשימת שורות שונה לגמרי (כל הרכיבים, לא רק אלה שבעץ) וכמעט
        # תמיד ריקה בעמודות התאריך. רק אם אין בלוק כזה בכלל (קבצים שמזוהים
        # רק דרך אסטרטגיה 2, כמו HPR) נופלים ל-PN_COL כניסיון סביר בלבד.
        if _dlp:
            pn_col_idx = _dlp["pn"]
        else:
            try:
                pn_col_idx = df.columns.get_loc(PN_COL)
            except Exception:
                pn_col_idx = 106
        if len(MONTH_COLS) > 0:
                        ...
                        if pd.notnull(qty) and qty != '' and qty != 'NaN':
                            try:
                                ...
                                if q_val > 0:
                                    ...
                                    # הכמות הגולמית מהקובץ (q_val) היא כבר הכמות הנכונה ליחידת-
                                    # מערכת/לתא הספציפי הזה - בדיוק כמו עמודת ה-SUM וערכי המטריצה
                                    # שכבר גילינו שלא צריך להכפיל בהם שוב. הכפלה נוספת ב-system_
                                    # multiplier כאן יצרה כפל-ספירה חמור להרכבות משותפות בין ענפים
                                    # (למשל 180+100 יצא בטעות 560 במקום 280).
                                    displayed_build_qty = q_val
                                    plan_rows.append({
    ...
    st.session_state["_custom_assembly_plan_project"] = PROJECT_ID

assembly_plan_df = st.session_state["custom_assembly_plan_df"]

# ==========================================================
# ==========================================================
# בלוק "לוח האספקה" (ETA לפי חודש) - _supply_range כבר זוהה למעלה (עם מעקב
# רמת ביטחון + אפשרות תיקון ידני - ראה BLOCK_DETECTION_INFO), משתמשים בו כאן.
# ==========================================================
if _supply_range:
...
def get_component_available_by_month(pn, target_ym, inv_cache=None, wip_cache=None):
    ...
    # שימוש במיפוי PN->שורה שכבר חושב פעם אחת (PN_ROW_INDEX), במקום לסנן את כל
    # הטבלה ולהמיר מחרוזות מחדש בכל קריאה - זה היה צוואר הבקבוק המרכזי כאן.
    row_idx = PN_ROW_INDEX.get(str(pn).strip())
    base_stock = safe_num(df.at[row_idx, STOCK_COL]) if row_idx is not None else 0.0
    ...
    # "כמה מהרכיב הזה כבר תפוס ב-WIP" - עוברים רק על ההרכבות שבאמת משתמשות
    # ברכיב הזה (COMPONENT_TO_ASSEMBLIES), לא על כל עמודות ההרכבה בקובץ.
    wip_committed = 0.0
    if wip_cache:
        for asm_col, qty_per in COMPONENT_TO_ASSEMBLIES.get(str(pn).strip(), []):
...
# ==========================================================
# ==========================================================
# תיקון קריטי: מסנני הליבה (חודש/רמה/הרכבה/סוג פריט/ספק/חיפוש) עוברים
# לחלונית קופצת (popover) באזור הראשי - **לא** בסיידבר יותר. הסיבה:
# בדיקות חוזרות במובייל הראו שכפתור פתיחת הסיידבר המובנה של Streamlit
# לא נגיש בפועל אצל המשתמש (לא נמצא, ולא הצליח גם ניסיון JS ידני
# למצוא/להפעיל אותו - כנראה חסימת קליק ע"י שכבה אחרת). הפתרון: אותו
# דפוס popover שכבר עובד באמינות לניווט בין הטאבים, מיושם עכשיו גם
# למסננים - כך שהם *תמיד* נגישים משורה קבועה באזור הראשי, בלי שום
# תלות במנגנון הכיווץ/הרחבה של הסיידבר. אי אפשר להשתמש באותו מפתח
# (key) פעמיים באותה הרצת סקריפט, ולכן הווידג'טים **הועברו**
# (לא שוכפלו) - הם כבר לא קיימים בסיידבר יותר.
st.markdown('<div class="nav-bar-label">🔍 מסננים (חודש / הרכבה / סוג פריט / חיפוש)</div>', unsafe_allow_html=True)
# תיקון (בעקבות משוב): אותו עיקרון כמו בבורר הפרויקטים - תווית הכפתור
# עצמו משתנה דינמית לפי כמה מסננים פעילים כרגע (נקרא מ-session_state, שכבר
# מכיל את הבחירות מהרצה קודמת בזכות ה-key על כל widget) - כדי שגם בלי
# לפתוח את הפופאובר יהיה ברור אם יש מסננים פעילים, ושבחירה קודמת נקלטה.
_active_filter_count = sum([
    st.session_state.get("selected_assembly", "הכל") != "הכל",
    st.session_state.get("selected_item_type", "הכל") != "הכל",
    st.session_state.get("selected_aw", "הכל") != "הכל",
    st.session_state.get("selected_search_item", "הכל") != "הכל",
])
_filters_popover_label = f"🔍 מסננים ({_active_filter_count} פעילים)" if _active_filter_count > 0 else "🔍 פתח את כל המסננים"
main_filters_popover = st.popover(_filters_popover_label, use_container_width=True)

with main_filters_popover:
    if st.button("🔄 רענן נתונים (נקה קאש)", help="לוחצים כאן אם עדכנת קובץ ב-GitHub והאפליקציה עדיין מציגה נתונים ישנים - זה מנקה את כל מטמון הנתונים, פורץ גם קאש CDN של GitHub עצמו, ומכריח טעינה מחדש אמיתית, בלי לצאת ל-Streamlit Cloud."):
        load_data.clear()
        fetch_all_inventory_records.clear()
        fetch_wip_records.clear()
        fetch_saved_projects.clear()
        fetch_cloud_assembly_plan.clear()
        try:
            load_all_projects_procurement.clear()
        except Exception:
            pass
        st.session_state.pop("custom_assembly_plan_df", None)
        st.session_state["_cache_bust_token"] = int(time.time())
        st.success("הקאש נוקה - טוען מחדש...")
        st.rerun()

    webhook_url = st.text_input("🔗 Teams / Slack Webhook URL (אופציונלי)", value="")
    supplier_options = ["אופק", "ספק פנימי", "רכש אחר", "אחר"]

    st.markdown("##### 🔍 מסננים מתקדמים")

    if st.button("🧹 איפוס כל המסננים (Clear All)"):
        ...
        st.success("✅ כל המסננים אופסו")
        st.rerun()

    # כל חודש שנמצא בפועל בבלוק "תוכנית העבודה" של הקובץ נכנס לרשימה - בלי סינון
    # לפי תאריך קבוע. פרויקטים שונים מתחילים בזמנים שונים (חלקם אפילו כבר באמצע
    # ריצה, עם חודשי עבר שכבר עברו) - תאריך-סף קבוע כמו "רק מ-09/2026" היה חוסם
    # בטעות פרויקטים כאלה לגמרי (למשל פרויקט שמתחיל בינואר 2025).
    month_options = {}
    for m in MONTH_COLS:
        if pd.notnull(m):
            try:
    ...
    if not month_options:
        st.error(
            "⚠️ לא נמצאו עמודות חודש תקפות בבלוק 'תוכנית העבודה' של הקובץ שנטען כעת. "
            "ייתכן שהקובץ ב-GitHub עודכן אך הקאש עדיין מגיש גרסה ישנה/שונה - "
            "נסה את הכפתור '🔄 רענן נתונים (נקה קאש)' למעלה, "
            "או ודא שהקובץ אכן מכיל עמודות חודש תקינות."
        )
        st.stop()

    # אם ב-session_state נשארה בחירה ישנה שכבר לא קיימת באופציות הנוכחיות (למשל
    # אחרי החלפת קובץ המקור) - מנקים אותה כדי שהווידג'ט ייפול חזרה לברירת המחדל
    # במקום לגרום ל-KeyError בשורה הבאה.
    if st.session_state.get("selected_month_label") not in month_options:
        st.session_state.pop("selected_month_label", None)

    selected_month_label = st.selectbox("בחר חודש לניתוח חוסרים", list(month_options.keys()), key="selected_month_label")
    ...
    num_months_ahead = st.slider("📅 טווח מבט קדימה במספר חודשים", min_value=1, max_value=6, value=1, step=1, format="%d חודשים קדימה", key="num_months_ahead")

    level_options = ["הכל"] + sorted(list(set(str(assembly_levels[c]) for c in valid_assemblies)), key=lambda x: int(x) if x.isdigit() else 0)
    selected_level = st.selectbox("סינון לפי רמת עץ (BOM Level)", level_options, key="selected_level")
    ...
    # ==========================================================
    # שיפור: הערת מפתח (מכפיל) מול ההורה הישיר של ההרכבה הנבחרת
    # ==========================================================
    # כשההרכבה שנבחרה בפילטר מופיעה יותר מפעם אחת בתוך ההרכבה שמעליה
    # בעץ ה-BOM (qty_per_parent > 1), הכמויות שהוצגו עד עכשיו בטבלת
    # הפירוט היו "כמות לכל המערכת" ולא "כמות ליחידה אחת של ההרכבה שנבחרה"
    # - מה שמבלבל כשבודקים הרכבת-ביניים ולא את המוצר הסופי. עכשיו מוצגת
    # הערה מפורשת, וטבלת הפירוט (בהמשך) כבר מחלקת בפועל באותו מפתח.
    SELECTED_ASM_PARENT_QTY_FACTOR = 1.0
    if selected_assembly != "הכל":
        _sel_node = ASSEMBLY_BOM_TREE.get(selected_assembly, {})
        _sel_parent = _sel_node.get("parent")
        _sel_qty_per_parent = _sel_node.get("qty_per_parent", 1.0) or 1.0
        if _sel_parent and abs(_sel_qty_per_parent - 1.0) > 1e-9:
            SELECTED_ASM_PARENT_QTY_FACTOR = _sel_qty_per_parent
            _parent_desc = ASSEMBLY_BOM_TREE.get(_sel_parent, {}).get("desc", _sel_parent)
            st.caption(
                f"⚠️ שים לב: ההרכבה שבחרת מופיעה **{_sel_qty_per_parent:g}** פעמים "
                f"בהרכבה **{_sel_parent}** ({_parent_desc}). הכמויות שיוצגו למטה "
                f"כבר מחולקות במפתח הזה (יחסית ליחידה אחת של ההרכבה שבחרת, לא לכל המערכת)."
            )
    ...
    selected_item_type = st.selectbox("בחר סוג פריט (עמודה AS)", ["הכל"] + item_types, key="selected_item_type")

    aw_values = df[AW_COL].dropna().astype(str).unique().tolist() if AW_COL in df.columns else []
    aw_values = sorted(list(set(aw_values)))
    selected_aw = st.selectbox("סינון לפי ספק או BFE ", ["הכל"] + aw_values, key="selected_aw")

    item_choices = ["הכל"] + sorted([f"{str(r[PN_COL]).strip()} - {str(r[DESC_COL])}" for _, r in df.iterrows() if pd.notnull(r[PN_COL])])
    selected_search_item = st.selectbox("🔎 חיפוש מהיר (בחר או הקלד מק'ט/תיאור)", item_choices, key="selected_search_item")
    search_pn = selected_search_item.split(" - ")[0] if selected_search_item != "הכל" else "הכל"

# משוב חזותי מיידי על המסננים הפעילים - מוצג תמיד באזור הראשי, גם כשהפופאובר
# סגור (התנהגות ברירת מחדל תקינה של Streamlit - פופאובר לא נסגר לבד אחרי
# בחירה). בלי זה אין דרך לדעת מבחוץ שבחירת מסנן נקלטה בלי לפתוח מחדש.
_active_filters_parts = [f"חודש: {selected_ym}"]
if selected_assembly != "הכל":
    _active_filters_parts.append(f"הרכבה: {selected_assembly}")
if selected_item_type != "הכל":
    _active_filters_parts.append(f"סוג פריט: {selected_item_type}")
if selected_aw != "הכל":
    _active_filters_parts.append(f"ספק: {selected_aw}")
if search_pn != "הכל":
    _active_filters_parts.append(f"חיפוש: {search_pn}")
st.caption("🔎 מסננים פעילים כרגע: " + " | ".join(_active_filters_parts))
...
with main_filters_popover:
    st.divider()
    st.markdown("##### 📥 עדכון ETA וכמות אספקה מקובץ ספק")
    ...
    uploaded_eta_file = st.file_uploader("העלה קובץ ETA (עמודות: PN, ETA, Qty)", type=["xlsx", "xls"], key="eta_uploader")
    if uploaded_eta_file is not None:
        try:
            eta_df_sup = pd.read_excel(uploaded_eta_file)
            if st.button("⚡ עדכן ETA וכמות אספקה"):
                eta_count = 0
                with st.spinner(f"מעדכן {len(eta_df_sup)} שורות..."):
                    for _, s_row in eta_df_sup.iterrows():
                        ...
                        if p_code and p_code != 'nan' and new_eta and new_eta not in ["nan", "NaT", "None"]:
                            ...
                            if row_ok:
                                eta_count += 1
                st.success(f"עודכנו בהצלחה ETA וכמויות אספקה עבור {eta_count} שורות!")
        except Exception as e:
...
@st.cache_data(ttl=60)
def calculate_mrp_breakdown_cached(project_id_for_cache, target_yms_tuple, sim_extra_stock_items_tuple, active_plan_df):
    sim_extra_stock_dict = dict(sim_extra_stock_items_tuple)
    inv_cache = fetch_all_inventory_records(project_id_for_cache)
    wip_cache = fetch_wip_records(project_id_for_cache)
    ...
    for idx, row in temp_df.iterrows():
        ...
        is_short_or = False
        # תיקון קריטי: סף אפסילון קטן נגד "רעש" של נקודה צפה. חישובי
        # Python על מספרים עשרוניים (float) יכולים להפיק ערכים כמו
        # -0.0000001 במקום 0 בדיוק, בגלל אופן הייצוג הבינארי של שברים
        # עשרוניים - זה נראה לעין כאילו "0.000" בתצוגה (בגלל עיגול
        # התצוגה), אבל טכנית עדיין שלילי, ולכן עובר את הסינון "< 0" 
        # ומוצג בטבלה כאילו יש חוסר אמיתי, כשבפועל אין. משתמשים בסף
        # קטן (EPSILON) כדי שרק חוסרים אמיתיים (לא רעשי עיגול) ייספרו.
        #
        # תיקון (בעקבות דיווח בפועל): EPSILON=1e-6 היה קטן מדי - נמצאו
        # בפועל שורות עם "סך חוסר" שמוצג כ-0.000 (רעש של בערך 0.0001-
        # 0.0005, לא 0.000001) שעדיין עברו את הסף הישן ונכנסו לטבלה.
        # מכיוון שהכמויות כאן הן תמיד ספירות פריטים פיזיים (לא צריכות
        # להיות משמעותיות ברמת עשירית/מאית האחוז), סף של 0.01 בטוח
        # לחלוטין מפני הסתרת חוסר אמיתי, ומספיק גדול כדי לסנן את כל
        # רעשי העיגול שנצפו בפועל.
        EPSILON = 0.01
    ...
    mrp_shortages = temp_df[temp_df['Monthly_Balance'] < -1e-6].copy()
...
def calculate_mrp_breakdown(sim_extra_stock=None, target_yms=None, plan_df_override=None):
    ...
    res = calculate_mrp_breakdown_cached(PROJECT_ID, tuple(target_yms), tuple(sorted(sim_extra_stock.items())), active_plan)
    res_df = res.copy()
...
def compute_shared_executable_plan(target_yms, assemblies, inv_cache=None, wip_cache=None):
    ...
    for ym in chronological_yms:
        ...
        def get_pool(pn):
            if pn not in pool_cache:
                if pn in ASSEMBLY_BOM_TREE:
                    ...
                    # תת-ההרכבה חייבת להיות מוכנה בחודש הקודם, לא באותו חודש
                    # שבו האב שלה נצרך - בדיוק כמו רכיב רכש עם ETA.
                    sub_max_new, _ = compute_max_buildable(pn, _shift_ym(ym, -1), inv_cache, wip_cache)
        ...
        for asm_col in priority_order:
            ...
            # raw_build (מ-Raw_Build_Qty) הוא כבר הכמות הנכונה בפועל להרכבה הזו -
            # לא צריך להכפיל שוב במקדם המערכת (אותה סיבה בדיוק שתיקנו בבניית
            # assembly_plan_df למעלה - זו הייתה יוצרת כפל-ספירה חמור להרכבות
            # משותפות בין ענפים).
            discrete_plan_build = raw_build
...
def _shift_ym(ym, delta_months):
    """מזיז מחרוזת חודש (YYYY-MM) קדימה/אחורה במספר חודשים. משמש לאכיפת עיכוב
    ייצור: תת-הרכבה חייבת להיות גמורה ומוכנה במלאי חודש *לפני* שההרכבה שמעליה
    (האב) יכולה לצרוך אותה - בדיוק כמו רכיב רכש עם ETA (לא מותר להשתמש
    ברכיב באותו חודש שהוא מגיע/נגמר בו). בלי זה, המערכת הייתה בטעות מאפשרת
    לאב לרדת לייצור באותו חודש שבו תת-ההרכבה שלו רק הופכת זמינה."""
    try:
        dt = pd.to_datetime(ym + "-01")
        dt = dt + pd.DateOffset(months=delta_months)
...
def check_hierarchical_ctb(asm_pn, requested_qty, target_ym, inv_cache=None, wip_cache=None, _visited=None, _claimed=None):
    ...
    if asm_pn in ASSEMBLY_DIRECT_COMPONENTS:
        sys_factor = ASSEMBLY_SYSTEM_FACTORS.get(asm_pn, 1)
        for comp_pn, qty_per, comp_desc, base_stock_check in ASSEMBLY_DIRECT_COMPONENTS[asm_pn]:
    ...
    for child_pn in ASSEMBLY_CHILDREN.get(asm_pn, []):
        ...
        if net_needed > 0:
            # תת-הרכבה חייבת להיות מוכנה בחודש שלפני חודש הצריכה של האב - לא
            # באותו חודש (בדיוק כמו רכיב רכש עם ETA).
            child_target_ym = _shift_ym(target_ym, -1)
            blockers.extend(check_hierarchical_ctb(child_pn, net_needed, child_target_ym, inv_cache, wip_cache, _visited, _claimed))
...
def _max_buildable_from_direct_components(asm_pn, target_ym, inv_cache, wip_cache):
    ...
    if asm_pn not in ASSEMBLY_DIRECT_COMPONENTS:
...
def _max_buildable_from_subassemblies(asm_pn, target_ym, inv_cache, wip_cache, visited, memo):
    max_qty = float('inf')
    limiting_component = None
    ...
    for child_pn in ASSEMBLY_CHILDREN.get(asm_pn, []):
        ...
        child_wip = wip_cache.get(child_pn, 0.0)
        # תת-ההרכבה חייבת להיות מוכנה בחודש הקודם, לא באותו חודש - ראה הסבר ב-_shift_ym.
        child_max_new, _ = compute_max_buildable(child_pn, _shift_ym(target_ym, -1), inv_cache, wip_cache, visited, memo)
...
def compute_max_buildable(asm_pn, target_ym, inv_cache=None, wip_cache=None, _visited=None, _memo=None):
    if inv_cache is None:
        inv_cache = fetch_all_inventory_records(PROJECT_ID)
    if wip_cache is None:
        wip_cache = fetch_wip_records(PROJECT_ID)
    if _visited is None:
        _visited = set()
    # _memo שומר תוצאות (הרכבה, חודש) שכבר חושבו - חוסך המון זמן בעצים עם
    # הרכבות משותפות (אותה הרכבה נגישה מכמה ענפים), כי בלי זה כל הרכבה משותפת
    # הייתה מחושבת מחדש בכל פעם שהיא מופיעה, ויכולה להתפוצץ אקספוננציאלית
    # בעץ עמוק/רחב עם הרבה שיתוף. בטוח לזכור כי inv_cache/wip_cache קבועים
    # לאורך כל הקריאה הזו (לא משתנים תוך כדי, בשונה מ-compute_shared_executable_plan).
    if _memo is None:
        _memo = {}
    if asm_pn in _visited:
        return float('inf'), None
    memo_key = (asm_pn, target_ym)
    ...
    sub_qty, sub_limiter = _max_buildable_from_subassemblies(asm_pn, target_ym, inv_cache, wip_cache, _visited, _memo)
    ...
    if max_qty == float('inf'):
        max_qty = 0.0
    result = (max(0.0, max_qty), limiting_component)
    _memo[memo_key] = result
    return result

# ==========================================================
# נתיב קריטי (Critical Path Method - CPM) - פונקציה עצמאית, לא תלוית UI.
# ==========================================================
ASSEMBLY_OPERATION_DURATION_DAYS = 2   # משך קבוע לכל פעולת הרכבה בעץ ה-BOM
MISSING_ETA_DEFAULT_DAYS = 60          # הנחת ברירת מחדל לרכיב-קצה בלי ETA: today + 60 יום

def calculate_critical_path(root_asm_pn, assembly_children=None, assembly_direct_components=None,
                             get_eta_func=None, inv_cache=None, wip_cache=None, today=None):
    """מחשב נתיב קריטי (Critical Path Method) לעץ ה-BOM שמתחת ל-root_asm_pn.

    כל הפרמטרים (מלבד root_asm_pn) אופציונליים ונופלים כברירת מחדל למבני
    הנתונים הגלובליים של האפליקציה (ASSEMBLY_CHILDREN / ASSEMBLY_DIRECT_COMPONENTS
    / get_first_supply_eta) - כדי שהפונקציה תעבוד "out of the box" בתוך האפליקציה,
    אבל גם תהיה ניתנת לבדיקה/שימוש חוזר עצמאי עם נתונים מוזרקים.

    פרמטרים:
      root_asm_pn                - מק"ט הרכבת השורש (המוצר הסופי) לחישוב.
      assembly_children           - dict: parent_pn -> [child_pn, ...]. ברירת מחדל: ASSEMBLY_CHILDREN.
      assembly_direct_components  - dict: asm_pn -> [(comp_pn, qty_per, desc, stock), ...]. ברירת מחדל: ASSEMBLY_DIRECT_COMPONENTS.
      get_eta_func                - פונקציה comp_pn -> ETA (str/Timestamp/None). ברירת מחדל: get_first_supply_eta (עם inv_cache).
      inv_cache                   - מטמון רשומות מלאי, מועבר ל-get_component_available_by_month/get_first_supply_eta כברירת המחדל.
      wip_cache                   - מטמון רשומות WIP, מועבר ל-get_component_available_by_month כברירת המחדל.
      today                       - תאריך עוגן (date). ברירת מחדל: date.today().

    בדיקת זמינות לרכיב-קצה (חומר גלם/רכש) - **מלאי פיזי אמיתי עכשיו**, לא
    "האם מופיע ברשימת חוסרים ביחס לאיזה חודש": משתמשים ב-
    get_component_available_by_month(comp_pn, target_ym=חודש נוכחי) - אם
    יוצא >0, יש מלאי זמין ממש עכשיו והרכיב לא חוסם, בלי קשר לביקוש עתידי
    כלשהו (זו שאלה נפרדת, של "מקסימום לבנייה"/CTB, לא של "מתי אפשר להתחיל
    לבנות"). זה נכון וישיר יותר מבדיקת "רשימת חוסרים" ביחס לחודש כלשהו,
    שיכולה להחמיץ רכיב עם מלאי אפס פשוט כי אין לו ביקוש רשום באותו חודש.

    שיטת החישוב - CPM קלאסי, שני מעברים על העץ:
      1. Forward Pass (Post-order - הילדים תמיד מחושבים לפני ההורה):
         - צומת-רכיב-קצה (עלה בעץ ה-BOM - אין לו בנים משלו, כלומר רכיב
           רכש/חומר גלם אמיתי): אם יש מלאי פיזי עכשיו (ראה למעלה) - ES=EF=today.
           אחרת: EF=ETA האפקטיבי (עם עיגול לתחילת החודש שאחרי - ראה
           _effective_component_date), או today+60 יום אם אין ETA תקין.
         - צומת-הרכבה (יש לו בנים ב-BOM - רכיבים ישירים ו/או תתי-הרכבות):
           **אין להרכבה שום ETA עצמאי - שדה ETA שמופיע עבורה (אם בכלל) לא
           נבדק ולא משמש בחישוב בשום שלב.** EF שלה נגזר אך ורק מהבנים שלה:
           ES = המקסימום (המאוחר ביותר) מבין EF של כל הבנים (רכיבים ישירים
           ותתי-הרכבות כאחד) - קודם מוצאים את המקסימום, ורק אז, בשלב נפרד,
           מוסיפים את זמן ההרכבה. EF = ES + 2 ימים (משך הרכבה קבוע).
      2. Backward Pass (מהשורש כלפי מטה): LF(שורש)=EF(שורש) (השורש קובע את
         תאריך היעד של עצמו - אין "איחור" ביחס לשום דדליין חיצוני), ומכאן
         LS/LF מחושבים אחורה לכל הילדים. Slack(node) = LS(node) - ES(node).
         Slack=0 <=> הצומת נמצא על הנתיב הקריטי - כל עיכוב בו מעכב ישירות
         את תאריך היעד הסופי.

    מחזיר dict:
      {
        "target_date": date,           # EF של השורש - תאריך היעד המחושב למוצר הסופי
        "critical_path": [node, ...],  # הצמתים על הנתיב הקריטי, מהשורש כלפי מטה
        "nodes": {node_id: node, ...}, # כל צומת בעץ (גם לא-קריטיים) - ראה מבנה node למטה
        "root_id": node_id,
      }

    כל node הוא dict עם: id, pn, desc, kind ("assembly"/"component"), parent_id,
    children_ids, es, ef, ls, lf, duration_days, slack_days, is_critical,
    is_eta_assumption (True אם EF מבוסס על ברירת המחדל של 60 יום, לא ETA אמיתי -
    רלוונטי רק לצמתים מסוג "component"; להרכבה זה תמיד False, כי היא לא
    משתמשת ב-ETA בכלל).

    שימו לב: אותו PN יכול להופיע כמה פעמים בעץ (רכיב/תת-הרכבה משותפים בין
    ענפים שונים) - לכן המפתח ב-nodes הוא node_id ייחודי-למופע, לא PN.
    """
    if assembly_children is None:
        assembly_children = ASSEMBLY_CHILDREN
    if assembly_direct_components is None:
        assembly_direct_components = ASSEMBLY_DIRECT_COMPONENTS
    if get_eta_func is None:
        get_eta_func = lambda pn: get_first_supply_eta(pn, inv_cache)
    if today is None:
        today = date.today()
    ...
    def _effective_component_date(comp_pn):
        """תאריך זמינות אפקטיבי לרכיב רכש - ראה דוקסטרינג הפונקציה למעלה.
        מחזיר (date, is_assumption, reason) - reason ∈ {"stock","eta","assumption"}:
        "stock" = יש מלאי פיזי זמין עכשיו (לא רכיב חוסם בכלל - התאריך שלו
        נקבע כ-today רק כי הוא לא מעכב שום דבר, לא כי יש לו איזה "ETA של
        today"). "eta"/"assumption" - ראה למטה.

        כלל תזמון MRP קריטי (זהה בדיוק ל-get_component_available_by_month:
        "manual_eta_ym < target_ym") - אספקה שמגיעה בחודש קלנדרי M זמינה
        לייצור רק החל מ-M+1 (ה-1 בחודש), בלי קשר לאיזה יום בתוך M היא הגיעה
        בפועל בקובץ. כלומר "הרכיב מגיע באוגוסט" => "ההרכבה יכולה לעלות
        לקווים רק ב-1.9", לא באוגוסט עצמו - גם אם ה-ETA הגולמי נופל על יום
        ספציפי מוקדם בתוך אוגוסט. בלי זה, נתיב קריטי היה "מזכה" רכיבים
        בזמינות מוקדמת מדי, לא עקבי עם שאר חישובי הזמינות/חוסרים באפליקציה.
        """
        try:
            avail_now = get_component_available_by_month(comp_pn, _today_ym, inv_cache=inv_cache, wip_cache=wip_cache)
        except Exception:
            avail_now = 0.0
        if avail_now > 0:
            return today, False, "stock"
        eta_raw = get_eta_func(comp_pn)
        if eta_raw and str(eta_raw).strip() not in ("", "None", "NaT", "nan", "בדיקה נדרשת"):
            try:
                parsed = pd.to_datetime(eta_raw)
                if pd.notnull(parsed):
                    usable = (parsed.to_period("M") + 1).to_timestamp().date()
                    # תיקון (בעקבות משוב): אם ה-ETA כבר עבר (החודש הרשום כבר חלף)
                    # אבל הרכיב עדיין לא הופיע בפועל כמלאי פיזי זמין (אחרת היינו
                    # כבר יוצאים למעלה עם reason="stock") - כנראה שה-ETA הזה כבר
                    # לא אמין/לא מתעדכן. במקום "לזכות" את ההרכבה בתאריך שכבר עבר
                    # (מה שהיה נותן לה, באופן שגוי, זמינות "מהעבר"), מתייחסים
                    # לרכיב בדיוק כאילו אין לו ETA בכלל - אותה הנחת ברירת מחדל
                    # (today+60) כמו רכיב בלי ETA רשום לגמרי. reason ייעודי
                    # "eta_expired" מבדיל בתצוגה בין "אין ETA בכלל" לבין "היה
                    # ETA אבל הוא כבר עבר" - כדי לא להסתיר מהמשתמש שיש כאן נתון
                    # שדורש בדיקה/עדכון בקובץ המקור.
                    if usable < today:
                        return today + timedelta(days=MISSING_ETA_DEFAULT_DAYS), True, "eta_expired"
                    return usable, False, "eta"
            except Exception:
                pass
        return today + timedelta(days=MISSING_ETA_DEFAULT_DAYS), True, "assumption"

    def _build(pn, parent_id, _path=frozenset()):
        node_id = _new_id()
        node = {
            "id": node_id, "pn": pn,
            "desc": ASSEMBLY_BOM_TREE.get(pn, {}).get("desc", pn),
            "parent_id": parent_id, "children_ids": [],
        }
        nodes[node_id] = node

        # הגנה מפני מעגליות בנתוני BOM פגומים - לא אמור לקרות במבנה תקין
        # (העץ נבנה מ-level stack שמונע זאת), אבל ליתר ביטחון לא נכנסים
        # ללולאה אינסופית: מתייחסים למופע החוזר כאל רכיב-קצה עם ETA/הנחה.
        if pn in _path:
            eta_date, is_assumption, _reason = _effective_component_date(pn)
            node.update({
                "kind": "component", "es": today, "ef": eta_date,
                "duration_days": (eta_date - today).days, "is_eta_assumption": is_assumption,
                "availability_reason": _reason,
            })
            return node_id
        _path = _path | {pn}

        child_efs = []
        _added_child_pns = set()
        for child_pn in assembly_children.get(pn, []):
            child_id = _build(child_pn, node_id, _path)
            node["children_ids"].append(child_id)
            child_efs.append(nodes[child_id]["ef"])
            _added_child_pns.add(child_pn)

        for comp_pn, qty_per, comp_desc, base_stock in assembly_direct_components.get(pn, []):
            # תיקון קריטי: שורת-רכיב שה-PN שלה זהה להרכבה עצמה (self-reference,
            # למשל שורת "סה"כ מערכת" בקובץ שנכנסת בטעות תחת העמודה של עצמה)
            # - היא לעולם לא רכיב אמיתי, ולא מייצגת שום קשר BOM אמיתי. בלי
            # הדילוג הזה, ה-PN "נכנס לעצמו" (מזוהה כתת-הרכבה לפי ASSEMBLY_BOM_TREE
            # ומנגנון ההגנה מפני מעגליות תופס את זה ומטפל בו כרכיב-קצה מזויף
            # בלי ETA - "שרשרת" שקוצה מיד בחזרה לשורש עצמו, בלי להגיע לרכיב
            # האמיתי שבאמת קובע את התאריך).
            if comp_pn == pn:
                continue
            # stock=9999999 (סנטינל) מסמן ששורה זו היא בעצם מק"ט של תת-הרכבה
            # (מטופלת כבר למעלה דרך assembly_children), לא רכיב רכש אמיתי -
            # אותה מוסכמה בדיוק כמו בשאר האפליקציה (check_hierarchical_ctb,
            # _max_buildable_from_direct_components). קריטי לדלג עליה כאן: זו
            # אחת הדרכים לאכוף את הכלל "אין ETA עצמאי להרכבות".
            if base_stock >= 9000000:
                continue
            if comp_pn in _added_child_pns:
                continue  # כבר טופל למעלה כתת-הרכבה - למנוע כפילות/דריסה
            if comp_pn in ASSEMBLY_BOM_TREE:
                # תיקון קריטי: לא כל תת-הרכבה שמופיעה כשורה ישירה במטריצה
                # מסומנת בפועל עם ערך המלאי הסנטינלי (9999999) - זה תלוי
                # במבנה הקובץ הספציפי, ולא תמיד עקבי. לכן, בנוסף לבדיקת
                # הסנטינל למעלה, בודקים ישירות: אם ה-PN הזה הוא בעצמו מפתח
                # קיים ב-ASSEMBLY_BOM_TREE (כלומר יש לו עץ BOM משלו במקום
                # אחר בקובץ) - הוא בהכרח הרכבה, לא רכיב רכש, ולכן *לעולם*
                # לא משתמשים ב-ETA שאולי רשום לשורה שלו כאן. במקום זה נכנסים
                # אליו רקורסיבית ומחשבים את תאריך המוכנות שלו מהבנים שלו -
                # בדיוק כמו כל תת-הרכבה אחרת, ובלי קשר לאיזה הורה קרא לו.
                # זה מבטיח גם שאותו PN יחזיר תמיד את אותה תוצאה עקבית, לא
                # משנה דרך איזה הורה/מטריצה הוא הגיע לכאן.
                child_id = _build(comp_pn, node_id, _path)
                node["children_ids"].append(child_id)
                child_efs.append(nodes[child_id]["ef"])
                _added_child_pns.add(comp_pn)
                continue
            eta_date, is_assumption, _reason = _effective_component_date(comp_pn)
            comp_id = _new_id()
            nodes[comp_id] = {
                "id": comp_id, "pn": comp_pn, "desc": str(comp_desc),
                "parent_id": node_id, "children_ids": [],
                "kind": "component", "es": today, "ef": eta_date,
                "duration_days": (eta_date - today).days, "is_eta_assumption": is_assumption,
                "availability_reason": _reason,
            }
            node["children_ids"].append(comp_id)
            child_efs.append(eta_date)

        es = max(child_efs) if child_efs else today
        ef = es + timedelta(days=ASSEMBLY_OPERATION_DURATION_DAYS)
        node.update({
            "kind": "assembly", "es": es, "ef": ef,
            "duration_days": ASSEMBLY_OPERATION_DURATION_DAYS, "is_eta_assumption": False,
            "availability_reason": None,
        })
        return node_id

    root_id = _build(root_asm_pn, None)
    target_date = nodes[root_id]["ef"]

    # --- Backward Pass: LF/LS/Slack, מהשורש כלפי מטה ---
    nodes[root_id]["lf"] = nodes[root_id]["ef"]
    nodes[root_id]["ls"] = nodes[root_id]["lf"] - timedelta(days=nodes[root_id]["duration_days"])
    ...
    while _stack:
        _nid = _stack.pop()
        _order.append(_nid)
        _stack.extend(nodes[_nid]["children_ids"])

    for _nid in _order:
        _node = nodes[_nid]
        for _cid in _node["children_ids"]:
            _cnode = nodes[_cid]
            _cnode["lf"] = _node["ls"]
            _cnode["ls"] = _cnode["lf"] - timedelta(days=_cnode["duration_days"])

    for _node in nodes.values():
        _node["slack_days"] = (_node["ls"] - _node["es"]).days
        _node["is_critical"] = _node["slack_days"] <= 0

    # --- שחזור הנתיב הקריטי: מהשורש כלפי מטה, בכל צומת עוברים לילד עם ה-EF
    # המאוחר ביותר - זה בדיוק הענף שקבע את ה-ES של ההורה, ולכן הוא זה שגורם
    # לעיכוב אם יש עיכוב. ---
    critical_path = []
    _nid = root_id
    while _nid is not None:
        _node = nodes[_nid]
        critical_path.append(_node)
        if not _node["children_ids"]:
            break
        _nid = max(_node["children_ids"], key=lambda cid: nodes[cid]["ef"])

    return {"target_date": target_date, "critical_path": critical_path, "nodes": nodes, "root_id": root_id}

_exec_wip_cache = fetch_wip_records(PROJECT_ID)
_exec_inv_cache = fetch_all_inventory_records(PROJECT_ID)
_exec_unique_shortage_items = breakdown_df["PN"].nunique() if not breakdown_df.empty else 0

# ==========================================================
# תיקון קריטי: "הרכבות מושפעות/חסומות" צריך לספור רק הרכבות שבאמת
# חסומות בפועל בטווח הנבחר - לא כל הרכבה שבאיזשהו שלב משתמשת ברכיב
# שנמצא בחוסר. הגרסה הקודמת ספרה "הרכבות ייחודיות שמופיעות בטבלת
# החוסרים" (breakdown_df) - אבל טבלה זו כוללת כל הרכבה שהרכיב שייך
# אליה, גם אם להרכבה הזו אין בכלל תוכנית ייצור/WIP בחודש הנבדק (ולכן
# היא לא באמת "חסומה" - אין לה שום דבר לבנות שיכול להיחסם מלכתחילה).
# לדוגמה שזוהתה בפועל: 6 הרכבות "משתמשות" ברכיבים חסרים, אבל רק 4
# מהן היו אמורות לייצר בפועל באוגוסט - 2 האחרות בלי שום תוכנית לחודש
# הזה, אז אין להן "חסימה" אמיתית.
# הפתרון: משתמשים ב-compute_shared_executable_plan (מחושב פעם אחת
# כאן, ומשותף גם לטאב 1 למטה - כדי לא לחשב פעמיים ולהבטיח עקביות
# מלאה בין הכרטיס העליון לתוכן הטאב) - הרכבה "חסומה" היא רק כזו עם
# raw_build>0 או WIP>0 (יש לה בכלל מה לבנות) *וגם* net_executable
# קטן מהיעד שנותר (בפועל לא ניתן לספק את כל התוכנית).
_exec_shared_plan = compute_shared_executable_plan(selected_target_yms, valid_assemblies, _exec_inv_cache, _exec_wip_cache)
_exec_blocked_assemblies_set = set()
for (_asm, _ym), _info in _exec_shared_plan.items():
    _remaining_target = max(0.0, _info["raw_build"] - _info["wip"])
    if _remaining_target > 1e-9 and _info["net_executable"] < _remaining_target - 1e-9:
        _exec_blocked_assemblies_set.add(_asm)
_exec_blocked_assemblies = len(_exec_blocked_assemblies_set)
...
<div class="exec-summary-strip">
    ...
    <div class="exec-stat">
        ...
        <div class="exec-stat-label">הרכבות חסומות בפועל (עם תוכנית ייצור)</div>
    </div>
    <div class="exec-stat">
        <div class="exec-stat-icon">🏭</div>
...
if nav_page == "📈 Executive Dashboard":
    ...
    # תיקון: משתמשים מחדש במאגרים ובתוכנית המשותפת שכבר חושבו למעלה
    # (פס תקציר המנהלים) - לא מחשבים שוב מאפס, וזה גם מבטיח שהמספר
    # שמוצג כאן בטאב תמיד עקבי לחלוטין למה שמוצג למעלה בכרטיסי הסיכום.
    wip_cache_dash = _exec_wip_cache
    inv_cache_dash = _exec_inv_cache
    ...
    shared_plan = _exec_shared_plan

    # תיקון קריטי: "הרכבות חסומות" סופר רק הרכבות עם תוכנית ייצור/WIP
    # אמיתיים בטווח הנבחר שבפועל לא ניתן לספק במלואם - לא כל הרכבה
    # שמשתמשת באיזשהו רכיב חסר בלי קשר לתוכנית שלה (ראו הסבר מפורט
    # למעלה ליד _exec_blocked_assemblies_set).
    blocked_assemblies = len(_exec_blocked_assemblies_set & set(assemblies_to_evaluate))
    ...
    if not dash_df.empty and len(dash_df) > 0:
        ...
        # תיקון: אם ההרכבה שנבחרה בפילטר מופיעה כמה פעמים בהרכבה שמעליה
        # (SELECTED_ASM_PARENT_QTY_FACTOR != 1) - "כמות נדרשת" תוצג כאן
        # מחולקת במפתח הזה, כך שהיא משקפת יחידה אחת של ההרכבה שבחרת,
        # ולא את הכמות ברמת המערכת כולה (כפי שהייתה מוצגת קודם). לא נוגע
        # ב"ביקוש מדויק"/"סך חוסר" - אלה נשארים הסכומים האמיתיים.
        display_source_df = dash_df.copy()
        if SELECTED_ASM_PARENT_QTY_FACTOR != 1.0:
            display_source_df["Qty_Per_Assembly"] = display_source_df["Qty_Per_Assembly"] / SELECTED_ASM_PARENT_QTY_FACTOR

        # תיקון קריטי: מסננים שורות שבהן להרכבה אין בכלל תוכנית ייצור
        # או WIP בטווח הנבחר (Assembly_Monthly_Build = 0 וגם אין WIP) -
        # שורות כאלה מציגות "0.000000" גם ב"ת. ייצור" וגם ב"ביקוש מדויק",
        # כי אין שום דרישה אמיתית מהרכיב הזה עבור ההרכבה הזו בחודש
        # הנבדק (ההרכבה רק "משתמשת" בו באופן עקרוני איפשהו בעץ, לא
        # שיש לה בפועל מה לבנות עכשיו) - בדיוק אותה בעיה שכבר תוקנה
        # ב"הרכבות חסומות" למעלה, רק שכאן היא השפיעה על שורות הטבלה
        # במקום על מונה ה-KPI. לא מסננים שורות "ללא שיוך" (רכיבים בלי
        # שיוך להרכבה כלשהי כלל) - חוסר כזה תמיד רלוונטי, לא תלוי בתוכנית.
        _has_plan_or_wip = (
            (display_source_df["Assembly"] == "ללא שיוך") |
            (display_source_df["Assembly_Monthly_Build"] > 1e-9) |
            (display_source_df["Assembly"].map(lambda a: wip_cache_dash.get(a, 0.0)) > 0)
        )
        display_source_df = display_source_df[_has_plan_or_wip]

        display_df = display_source_df[[
        ...
        with st.form("close_wip_form"):
            ...
            if st.form_submit_button("סגור WIP והוסף למלאי הזמין"):
                if is_finished:
                    ...
                    with st.spinner("סוגר WIP ומעדכן מלאי..."):
                        ok = save_inventory_record(wip_to_close, curr_stk + closing_qty, curr_eta, "התקבל", curr_sup, f"{curr_comm} | הושלם מייצור WIP בסך {closing_qty}", "WIP Close", webhook_url)
                        if ok:
                            ok = delete_wip_record(wip_to_close)
                    if ok:
                        st.success("✅ ה-WIP נסגר והמלאי עודכן!")
                        st.rerun()
    ...
    if hierarchy_blockers:
        ...
        with st.form("wip_form"):
            ...
            if st.form_submit_button("שמור ל-WIP בכל זאת"):
                if override_confirm:
                    with st.spinner("שומר ל-WIP..."):
                        ok = save_wip_record(wip_asm_choice, wip_qty_input)
                    if ok:
    ...
    if selected_pn != "הכל":
        ...
        with st.form("inventory_form"):
            ...
            if st.form_submit_button("שמור עדכון קבוע בענן"):
                ...
                with st.spinner("שומר את העדכון בענן..."):
                    ok = save_inventory_record(selected_pn, added_stock_input, str(eta_date), status, supplier, comment, "User", webhook_url, unit_price=manual_price)
                if ok:
...
elif nav_page == "🧩 עיון בהרכבה - רכיבים ישירים":
    st.markdown('<div class="section-title">🧩 עיון בהרכבה - רכיבים ישירים (Where-Used)</div>', unsafe_allow_html=True)
    st.caption("בחר הרכבה כדי לראות את כל הבנים (הרכיבים הישירים) שלה, עם אפשרות סינון לפי סוג פריט.")

    if not filtered_assembly_cols:
        st.info("אין הרכבות זמינות להצגה (בהתאם לסינון רמת העץ שבסיידבר).")
    else:
        lookup_col1, lookup_col2 = st.columns(2)
        with lookup_col1:
            lookup_assembly = st.selectbox(
                "בחר הרכבה",
                filtered_assembly_cols,
                format_func=lambda x: assembly_mapping.get(x, x),
                key="lookup_assembly_select"
            )
        with lookup_col2:
            lookup_item_type = st.selectbox(
                "סנן לפי סוג פריט (ITEM_TYPE)",
                ["הכל"] + sorted(str(t) for t in item_types),
                key="lookup_item_type_select"
            )

        _lookup_rows = []
        for comp_pn, qty_per, comp_desc, base_stock in ASSEMBLY_DIRECT_COMPONENTS.get(lookup_assembly, []):
            comp_row_idx = PN_ROW_INDEX.get(comp_pn)
            comp_item_type = str(df.at[comp_row_idx, ITEM_TYPE_COL]) if (comp_row_idx is not None and ITEM_TYPE_COL in df.columns) else ""
            if lookup_item_type != "הכל" and comp_item_type != lookup_item_type:
                continue
            eta_ym, eta_qty = get_base_mrp_eta_and_qty(comp_pn)
            _lookup_rows.append({
                "מק\"ט (PN)": comp_pn,
                "תיאור (DESCRIPTION)": comp_desc,
                "כמות לקשר (QPA)": qty_per,
                "מלאי נוכחי (STOCK)": base_stock,
                "כמות הגעה ראשונה": eta_qty,
                "תאריך הגעה ראשונה (ETA)": eta_ym,
            })

        lookup_df = pd.DataFrame(_lookup_rows)
        if lookup_df.empty:
            st.warning("לא נמצאו רכיבים ישירים תואמים לסינון שנבחר עבור ההרכבה הזו.")
        else:
            st.caption(f"מציג {len(lookup_df)} רכיבים ישירים עבור: {assembly_mapping.get(lookup_assembly, lookup_assembly)}")
            st.dataframe(lookup_df, use_container_width=True, height=500)
    ...
    for item in updated_items:
        ...
        with col_u3:
            if st.button("🔄 UNDO", key=f"undo_{item.get('pn')}"):
                with st.spinner("מבטל..."):
                    ok = delete_inventory_record(item.get('pn'))
                if ok:
                    st.success(f"✅ בוטל: {item.get('pn')}")
                    st.rerun()
            ...
            with st.form("edit_mgmt_form"):
                ...
                if st.form_submit_button("🗑️ אפס או עדכן"):
                    with st.spinner("מעדכן..."):
                        ok = delete_inventory_record(selected_mgmt_pn)
                        if ok and new_qty > 0:
                            ok = save_inventory_record(selected_mgmt_pn, new_qty, "", "פתוח", "אופק", "", "Tab 9", webhook_url)
                    if ok:
                        st.success(f"✅ עודכן: {selected_mgmt_pn}")
                        st.rerun()
    ...
    with st.expander(f"📥 ייבוא מרוכז של רשימת ספקים ({len(DEFAULT_SUPPLIER_MAP)} פריטים) - עדכון חד-פעמי ל-DB", expanded=False):
        ...
        if st.button("💾 עדכן את כל רשימת הספקים ב-DB עכשיו", key="bulk_supplier_import_btn"):
            with st.spinner(f"מעדכן {len(DEFAULT_SUPPLIER_MAP)} פריטים ב-DB..."):
    ...
    if not bulk_rows:
        ...
        if st.button("💾 שמור את כל השינויים ל-DB", key="bulk_save_btn"):
            changed_count = 0
            with st.spinner(f"בודק ושומר שינויים ({len(bulk_df)} שורות)..."):
            ...
            if changed_count > 0:
                ...
                st.info("לא זוהו שינויים לשמירה (או שהשמירה נכשלה - ראה שגיאה למעלה אם הופיעה).")
...
elif nav_page == "🌳 עץ הרכבות (BOM)":
    # ==========================================================
    # תרשים עץ הרכבות היררכי (BOM Tree) - לפי הפרויקט הנטען כרגע
    # ==========================================================
    # משתמש ב-ASSEMBLY_BOM_TREE / ASSEMBLY_CHILDREN שכבר בנויים ומאומתים
    # מעץ ה-BOM האמיתי מהקובץ - לא בונה שום היררכיה חדשה, רק מצייר את
    # מה שכבר קיים. כל מלבן = הרכבה אחת (PN + תיאור), עם תווית "X (כמות)"
    # ליד קו החיבור להורה שלה, שמייצגת את כמות הקשר (qty_per_parent) -
    # כמה יחידות של ההרכבה הזו נדרשות ליחידה אחת של ההורה שמעליה.
    #
    # תיקון (בעקבות משוב): (1) רמה 0 (ראש העץ) הייתה מוצגת למטה במקום
    # למעלה - הסיבה: קביעת range מפורש ביחד עם autorange="reversed" בו
    # זמנית סותרת את עצמה ב-Plotly (ה-range המפורש גובר, ומבטל בפועל
    # את ההיפוך). התיקון: לא להשתמש ב-autorange כלל - במקום זה, לתת
    # ל-range עצמו סדר הפוך (מקסימום ואז מינימום), שזו הדרך הנכונה
    # לקבל ציר הפוך יחד עם טווח מוגדר. (2) הטקסט לא התאים לגודל המלבן -
    # התיקון: קנה מידה קבוע (פיקסלים ליחידה), כך שגודל המלבן בפיקסלים
    # ידוע במדויק, ואז ה-PN (מודגש, גדול) והתיאור (קטן, עם גלישת שורות
    # ידנית לפי textwrap להתאמה מדויקת לרוחב הידוע) תמיד נכנסים בתוכו.
    st.markdown('<div class="section-title">🌳 עץ הרכבות היררכי (BOM)</div>', unsafe_allow_html=True)
    st.caption("כל מלבן: קוד הרכבה (מודגש) + תיאור. המספר ליד קו החיבור להורה = כמות הקשר (כמה יחידות נדרשות ליחידה אחת של ההורה). רמה 0 (ראש העץ) למעלה.")

    import textwrap as _textwrap

    def _compute_bom_tree_layout():
        positions = {}
        next_x = [0.0]

        def _assign_x(pn, depth_guard):
            if pn in depth_guard:
                return next_x[0]
            depth_guard = depth_guard | {pn}
            children = ASSEMBLY_CHILDREN.get(pn, [])
            if not children:
                x = next_x[0]
                next_x[0] += 1.65  # מרווח מוגדל בהתאם למלבנים הרחבים יותר (ראה BOX_W למטה)
                positions[pn] = x
                return x
            child_xs = [_assign_x(c, depth_guard) for c in children]
            x = sum(child_xs) / len(child_xs)
            positions[pn] = x
            return x

        roots = [pn for pn, info in ASSEMBLY_BOM_TREE.items() if info.get("parent") is None]
        for r in sorted(roots):
            _assign_x(r, set())
        return positions

    _bom_positions = _compute_bom_tree_layout()

    if not _bom_positions:
        st.info("לא נמצא עץ הרכבות לפרויקט הנוכחי.")
    else:
        # קנה מידה קבוע (פיקסלים ליחידת x/y) - כך שאפשר לחשב במדויק כמה
        # תווים נכנסים בכל מלבן, בלי תלות ברוחב המסך של המשתמש.
        # תיקון (בעקבות משוב): התיאור עדיין גלש מחוץ למלבן - הגדלתי
        # משמעותית את המלבן עצמו (רוחב וגובה), ואת מספר השורות המותרות
        # לתיאור מ-2 ל-3 (הוא לא חייב להיכנס בשורה אחת), עם חישוב מחדש
        # של תווים-לשורה לפי הרוחב החדש - כך שגם תיאורים ארוכים נכנסים.
        # תיקון נוסף (בעקבות משוב): הקווים בין הורה לילדים היו אלכסוניים
        # ומתכנסים לנקודה אחת עם מעט רווח - זה נראה "מבולגן" וגם חתך
        # דרך המלבנים. עכשיו זה עץ אורגני קלאסי: קו אנכי יורד מההורה,
        # קו אופקי אחד ("אוטובוס") שמחבר את כל הילדים באותה רמה, וקו
        # אנכי לכל ילד מהאוטובוס למלבן שלו - בדיוק כמו בתרשים ארגוני
        # רגיל. בנוסף הגדלתי משמעותית את הרווח האנכי בין רמות, כדי
        # שיהיה מקום נקי לזווית החיבור בלי להיצמד למלבנים.
        # תיקון (בעקבות משוב נוסף): המלבנים היו מאורכים (גבוהים) והכיתוב
        # עדיין גלש - עכשיו מרחיבים אותם משמעותית (במקום להגביה), עם
        # תיאור בשתי שורות בלבד (לא שלוש) - הרוחב הנוסף נותן הרבה יותר
        # מקום לתו בכל שורה, כך שהתיאור נכנס בנוחות גם בפחות שורות.
        # תיקון נוסף (בעקבות משוב): עדיין יחס גובה/רוחב היה קרוב מדי לריבוע -
        # עכשיו המלבן רחב משמעותית ונמוך יותר (BOX_W גדל, BOX_H קטן), כדי
        # שה-PN (בדרך כלל נכנס עכשיו בשורה אחת) והתיאור באמת "יושבים" לרוחב
        # המלבן ולא מרגישים דחוסים לגובה. מרווח העמודות (next_x, למעלה)
        # הוגדל בהתאם כך שמלבנים רחבים יותר לא ייגעו זה בזה בצדדים.
        UNIT_PX_X = 320
        UNIT_PX_Y = 280
        BOX_W, BOX_H = 1.35, 0.6
        BOX_W_PX = BOX_W * UNIT_PX_X
        PN_CHARS_PER_LINE = max(6, int(BOX_W_PX / 8.6))     # גופן מודגש וגדול יותר -> פחות תווים לשורה
        DESC_CHARS_PER_LINE = max(8, int(BOX_W_PX / 5.6))   # גופן קטן יותר -> יותר תווים לשורה
        DESC_MAX_LINES = 2

        max_x = max(_bom_positions.values()) if _bom_positions else 1
        max_y = max(info.get("level", 0) for info in ASSEMBLY_BOM_TREE.values())

        fig_tree = go.Figure()
        shapes, annotations = [], []

        # ---------- קווי חיבור הורה-ילדים בסגנון "אוטובוס" אורתוגונלי ----------
        # מצוירים לפני המלבנים כדי שהמלבנים "יעלו" חזותית מעליהם.
        for parent_pn, children_pns in ASSEMBLY_CHILDREN.items():
            if parent_pn not in _bom_positions:
                continue
            valid_children = [c for c in children_pns if c in _bom_positions]
            if not valid_children:
                continue

            parent_x = _bom_positions[parent_pn]
            parent_level = ASSEMBLY_BOM_TREE.get(parent_pn, {}).get("level", 0)
            parent_bottom_y = parent_level + BOX_H / 2

            child_top_ys = [ASSEMBLY_BOM_TREE.get(c, {}).get("level", parent_level + 1) - BOX_H / 2 for c in valid_children]
            bus_y = parent_bottom_y + (min(child_top_ys) - parent_bottom_y) / 2  # אמצע הפער בין ההורה לילדים

            # קו אנכי (גזע) מההורה לאוטובוס
            shapes.append(dict(
                type="line", x0=parent_x, y0=parent_bottom_y, x1=parent_x, y1=bus_y,
                line=dict(color="rgba(148,163,184,0.85)", width=1.8)
            ))

            child_xs = [_bom_positions[c] for c in valid_children]
            bus_x0, bus_x1 = min(child_xs + [parent_x]), max(child_xs + [parent_x])
            # קו האוטובוס האופקי - מחבר את כל הילדים באותה רמה
            shapes.append(dict(
                type="line", x0=bus_x0, y0=bus_y, x1=bus_x1, y1=bus_y,
                line=dict(color="rgba(148,163,184,0.85)", width=1.8)
            ))

            for c in valid_children:
                cx = _bom_positions[c]
                c_top_y = ASSEMBLY_BOM_TREE.get(c, {}).get("level", parent_level + 1) - BOX_H / 2
                # קו אנכי (ירידה) מהאוטובוס לכל ילד
                shapes.append(dict(
                    type="line", x0=cx, y0=bus_y, x1=cx, y1=c_top_y,
                    line=dict(color="rgba(148,163,184,0.85)", width=1.8)
                ))
                # תיקון: קודם נלקח qty_per_parent מ-ASSEMBLY_BOM_TREE[c] - ערך
                # *יחיד* לכל PN (לא לפי קשר ספציפי), שבמקרה של תת-הרכבה
                # משותפת בין כמה הורים (אותו PN מופיע ביותר מהרכבה אחת בעץ -
                # אותר בפועל למשל ב-HPR.xlsx) הציג את אותה כמות שגויה עבור
                # כל ההורים, במקום הכמות האמיתית של הקשר (parent_pn, c) הזה
                # בדיוק. עכשיו נלקח ישירות מ-ASSEMBLY_EDGE_QTY לפי (הורה,ילד) -
                # אותו מקור אמת שכל חישובי הכמות האמיתיים באפליקציה (CTB/
                # מקסימום לבנייה) כבר משתמשים בו - כך שהתווית תמיד נכונה
                # לקשר הספציפי המצויר, לא משנה כמה הורים יש לילד הזה.
                qty_lbl = ASSEMBLY_EDGE_QTY.get((parent_pn, c), 1.0) or 1.0
                annotations.append(dict(
                    x=cx, y=(bus_y + c_top_y) / 2,
                    text=f"X {qty_lbl:g}", showarrow=False,
                    font=dict(size=10, color=WARNING), bgcolor="rgba(0,0,0,0.55)",
                    borderpad=1
                ))

        # ---------- מלבנים ----------
        for pn, info in ASSEMBLY_BOM_TREE.items():
            if pn not in _bom_positions:
                continue
            x_, y_ = _bom_positions[pn], info.get("level", 0)
            shapes.append(dict(
                type="rect", x0=x_ - BOX_W / 2, x1=x_ + BOX_W / 2, y0=y_ - BOX_H / 2, y1=y_ + BOX_H / 2,
                line=dict(color=PRIMARY, width=1.6),
                fillcolor="rgba(79,70,229,0.16)" if info.get("level", 0) == 0 else "rgba(6,182,212,0.14)"
            ))

            # גלישת שורות ידנית מדויקת, מותאמת לרוחב הידוע של המלבן - כדי
            # שה-PN (מודגש) והתיאור (קטן) תמיד ייכנסו בתוך המלבן, לא יגלשו.
            pn_lines = _textwrap.wrap(str(pn), width=PN_CHARS_PER_LINE, max_lines=2, placeholder="…") or [str(pn)]
            desc_raw = str(info.get("desc", "")).strip()
            desc_lines = _textwrap.wrap(desc_raw, width=DESC_CHARS_PER_LINE, max_lines=DESC_MAX_LINES, placeholder="…") if desc_raw else []

            pn_html = "<br>".join(pn_lines)
            desc_html = "<br>".join(desc_lines)
            full_text = f"<b>{pn_html}</b>" + (f"<br><span style='font-size:10px'>{desc_html}</span>" if desc_html else "")

            annotations.append(dict(
                x=x_, y=y_, text=full_text, showarrow=False,
                font=dict(size=12), align="center",
                # second (smaller) style isn't per-line in plotly annotations, so
                # the description is rendered slightly smaller via <span> below
            ))

        fig_tree.update_layout(
            shapes=shapes, annotations=annotations,
            xaxis=dict(visible=False, range=[-1, max_x + 1]),
            # תיקון: לא autorange="reversed" (מתנגש עם range מפורש) - במקום
            # זה, נותנים ל-range עצמו סדר הפוך (מקס' ואז מינ') כדי שרמה 0
            # (y=0, המספר הקטן ביותר) תרד לראש התרשים.
            yaxis=dict(visible=False, range=[max_y + 0.8, -0.8]),
            template=PLOTLY_TEMPLATE,
            width=int((max_x + 2) * UNIT_PX_X),
            height=int((max_y + 1.6) * UNIT_PX_Y),
            margin=dict(t=10, b=10, l=10, r=10),
            plot_bgcolor="rgba(0,0,0,0)"
        )
        # תיקון (בעקבות משוב): (1) staticPlot=True מכבה את יכולת ה-pan/zoom
        # הפנימית של Plotly - בלעדיו, גרירה במגע (מובייל) "נלכדת" בתוך
        # התרשים במקום לגלול את העמוד, וזה בדיוק מה שמנע חזרה לתפריטים
        # למעלה אחרי גלילה לתוך העץ. (2) ה-CSS הגלובלי (הוגדר למעלה בקובץ)
        # הופך את מיכל התרשים לגלילה אופקית/אנכית רגילה - כך שבמסך צר
        # (טלפון) אפשר לגלול ולראות את כל העץ בלי שהוא נדחס ונהיה בלתי-קריא.
        st.plotly_chart(fig_tree, use_container_width=False, config={"staticPlot": True})
        st.caption(f"סה\"כ {len(_bom_positions)} הרכבות, {max_y + 1} רמות עומק. גוללים בתוך התרשים (למעלה/למטה/לצדדים) כדי לראות הכל - הגלילה הרגילה של העמוד לא נלכדת בתוכו.")

        # ==========================================================
        # אפשרות הורדה לעץ המוצר הגרפי - שני פורמטים:
        # (1) PNG - תמונה סטטית, נוחה לשיתוף/הדפסה/הטמעה במסמך. דורשת את
        #     חבילת kaleido (רינדור Plotly->תמונה בצד שרת) - לא תמיד מותקנת
        #     כברירת מחדל, לכן עטופה ב-try/except עם הודעה ברורה אם היא
        #     חסרה (יש להוסיף "kaleido" ל-requirements.txt של הפרויקט),
        #     במקום שהעמוד כולו ייפול אם החבילה לא זמינה.
        # (2) HTML אינטראקטיבי - לא תלוי ב-kaleido בכלל (עובד תמיד), נפתח
        #     בכל דפדפן, ושומר על יכולת זום/pan (בניגוד לתצוגה בתוך
        #     האפליקציה, שכבויה בכוונה ל-staticPlot בשביל גלילה נכונה בנייד).
        # ==========================================================
        _dl_col1, _dl_col2 = st.columns(2)
        with _dl_col1:
            try:
                _tree_png_bytes = fig_tree.to_image(format="png", scale=2)
                st.download_button(
                    "⬇️ הורדת עץ המוצר כתמונה (PNG)",
                    data=_tree_png_bytes,
                    file_name=f"bom_tree_{PROJECT_ID}.png",
                    mime="image/png",
                    key="bom_tree_png_dl",
                )
            except Exception:
                st.caption("⚠️ הורדה כ-PNG דורשת את חבילת `kaleido` (יש להוסיף ל-requirements.txt). עד אז אפשר להוריד כ-HTML לצד.")
        with _dl_col2:
            _tree_html_bytes = fig_tree.to_html(include_plotlyjs="cdn").encode("utf-8")
            st.download_button(
                "⬇️ הורדת עץ המוצר כקובץ HTML אינטראקטיבי",
                data=_tree_html_bytes,
                file_name=f"bom_tree_{PROJECT_ID}.html",
                mime="text/html",
                key="bom_tree_html_dl",
            )

elif nav_page == "🎯 נתיב קריטי":
    # ==========================================================
    # נתיב קריטי לכל הרכבה, עד לראש העץ - עץ נפתח + טבלת סיכום
    # ==========================================================
    # מבוסס על calculate_critical_path() - פונקציית CPM עצמאית (מוגדרת מעל,
    # ליד compute_max_buildable) שלא תלויה ב-Streamlit/UI, ומחזירה מבנה נתונים
    # נקי (ES/EF/LS/LF/Slack לכל צומת) שהטאב הזה רק מציג. ראה דוקסטרינג של
    # הפונקציה להסבר המלא על שיטת החישוב.
    #
    # כמו בגרסה הקודמת של הטאב: משתמשים ב-breakdown_df (רשימת החוסרים בפועל
    # שמוצגת בכל שאר האפליקציה) כדי לדעת אילו רכיבים באמת חוסמים - רכיב עם
    # מלאי מספיק לא נחשב חוסם, גם אם יש לו ETA עתידי רשום (הזמנה שגרתית לא
    # דחופה). זה מוזרק לפונקציה דרך shortage_pns.
    st.markdown('<div class="section-title">🎯 נתיב קריטי לכל הרכבה (עד לראש העץ)</div>', unsafe_allow_html=True)
    st.caption(
        "רק רכיבים שבאמת נמצאים בחוסר (מרשימת החוסרים הקיימת של המערכת) משפיעים על התאריך - רכיב שיש ממנו מלאי מספיק "
        "נחשב זמין כבר היום, גם אם יש לו ETA עתידי רשום (הזמנה שגרתית שלא דחופה). לכל הרכבה מתווספים יומיים זמן בנייה "
        "משלה, מעבר לזמינות הרכיבים/תתי-ההרכבות שלה - כך שעיכוב בהרכבה תחתונה מתגלגל ומצטבר כלפי מעלה עד לראש העץ. "
        "רכיב-קצה (חומר גלם/רכש) בלי ETA מוגדר כלל מקבל הנחת ברירת מחדל של 60 יום מהיום, ומסומן בבירור \"(הנחה)\" בתוצאה. "
        "להרכבות (יש להן בנים ב-BOM) אין ETA עצמאי בכלל - תאריך המוכנות שלהן נגזר תמיד מהבנים שלהן בלבד + 2 ימי הרכבה."
    )

    _cp_inv_cache = fetch_all_inventory_records(PROJECT_ID)
    _cp_wip_cache = _exec_wip_cache
    # תיקון קריטי (סבב 3): לא בודקים "האם מופיע ברשימת חוסרים ביחס לחודש
    # מסוים" (זה תלוי בביקוש הרשום לאותו חודש - רכיב שאין לו ביקוש בחודש
    # הראשון פשוט לא נכנס לרשימה, גם אם המלאי הפיזי שלו בפועל הוא אפס!).
    # במקום זה, לכל רכיב-קצה בודקים ישירות "כמה יש ממנו זמין פיזית עכשיו"
    # דרך get_component_available_by_month (אותה פונקציה שכל שאר האפליקציה
    # משתמשת בה) - אם התוצאה >0, יש מלאי אמיתי והרכיב לא חוסם; אחרת בודקים
    # את ה-ETA האמיתי שלו (עם עיגול לתחילת החודש הבא). זה נכון וישיר יותר -
    # לא תלוי בביקוש/תכנון של אף חודש ספציפי, ולא בפילטרים של שום טאב אחר.
    st.caption("🔎 לכל רכיב-קצה נבדק מלאי פיזי זמין בפועל עכשיו (לא תלוי בביקוש/תכנון של חודש מסוים) - אם אין, נבדק ה-ETA האמיתי שלו.")

    _cp_roots = [pn for pn, info in ASSEMBLY_BOM_TREE.items() if info.get("parent") is None]

    # מריצים את calculate_critical_path() בנפרד לכל הרכבת-שורש (יש בדרך כלל
    # כמה שורשים בקובץ - כל אחד עץ נפרד) - ומאחדים לתוך מבנה נתונים אחד
    # לתצוגה: nodes-per-root, ומק"טים על הנתיב הקריטי (לכל שורש) לצביעה.
    _cp_results = {}       # root_pn -> תוצאת calculate_critical_path()
    _cp_critical_pns = set()  # מק"טי הרכבה שנמצאים על נתיב קריטי (בכל שורש שבו הם מופיעים)
    for _root in _cp_roots:
        _res = calculate_critical_path(
            _root,
            inv_cache=_cp_inv_cache,
            wip_cache=_cp_wip_cache,
        )
        _cp_results[_root] = _res
        for _cp_node in _res["critical_path"]:
            if _cp_node["kind"] == "assembly":
                _cp_critical_pns.add(_cp_node["pn"])

    # ==========================================================
    # מיפוי רכיבי-קצה (רכש) שמשותפים בין כמה מוצרי-שורש (רמה 0) שונים -
    # לתצוגה בלבד (אינפורמטיבי), בלי לשנות בכלל את חישוב הנתיב הקריטי עצמו.
    # שים לב: הנתיב הקריטי לכל שורש מחושב היום *באופן עצמאי* לגמרי - הוא
    # לא "יודע" שאותו רכיב-קצה גם נדרש למוצר-שורש אחר, ולכן שני השורשים
    # יכולים כל אחד לראות את אותו המלאי/ETA כ"זמין לו" בלי שום תיאום ביניהם.
    # במקום לשנות את האלגוריתם (החלטה במפורש לא לעשות זאת - כל הקצאה בפועל
    # של מלאי מוגבל בין כמה מוצרים היא החלטה עסקית של המשתמש, לא משהו
    # שהאלגוריתם צריך "להחליט" בשקט), רק מסמנים בבירור ליד כל רכיב-קצה כזה
    # שהוא משותף גם לשורש/שורשים אחרים - כך שהמשתמש יידע שיש כאן החלטת
    # הקצאה שצריך לקבל בעצמו.
    # ==========================================================
    _cp_component_roots = {}  # comp_pn -> set(root_pn) - בכל שורש שבו הפריט מופיע כרכיב-קצה
    for _root, _res in _cp_results.items():
        for _node in _res["nodes"].values():
            if _node["kind"] == "component":
                _cp_component_roots.setdefault(_node["pn"], set()).add(_root)

    def _shared_root_note(node, current_root):
        """הערה קצרה (עם רווח מוביל) שמופיעה ליד רכיב-קצה שמשותף גם למוצר-
        שורש אחר - ראה הסבר למעלה. מחזיר מחרוזת ריקה אם הרכיב לא משותף,
        או שהוא לא רכיב-קצה בכלל."""
        if node["kind"] != "component":
            return ""
        _other_roots = _cp_component_roots.get(node["pn"], set()) - {current_root}
        if not _other_roots:
            return ""
        _other_desc = ", ".join(
            f"{r} ({ASSEMBLY_BOM_TREE.get(r, {}).get('desc', r)})" for r in sorted(_other_roots)
        )
        return f" ⚠️ פריט משותף גם למוצר-שורש {_other_desc} (רמה 0) - נדרשת החלטת הקצאה בין המוצרים"

    # מיפוי pn -> node "הגרוע ביותר" שלו (לכל הרכבה, אם היא מופיעה בכמה מקומות
    # בעץ - רכיב/תת-הרכבה משותפים בין ענפים שונים - כל מופע מחושב בנפרד ויכול
    # לצאת עם תאריך שונה, כי לכל ענף שרשרת אחרת מתחתיו). התיקון כאן קריטי:
    # בעבר נשמר תמיד המופע *הראשון* שנמצא (setdefault) - מה שיכול היה להראות
    # בטבלה תאריך מוקדם/לא-מייצג, בזמן שבעץ האינטראקטיבי (שמציג כל מופע
    # בנפרד, במקום שלו בפועל) מופיע מופע אחר, גרוע יותר, של אותו מק"ט. עכשיו
    # תמיד נשמר המופע עם ה-EF המאוחר ביותר - כדי שהטבלה תמיד תשקף את התרחיש
    # הכי גרוע (=הכי נכון להצגה, כי זה מה שבאמת עלול לעכב), ותהיה עקבית עם
    # מה שרואים בעץ.
    # תיקון קריטי (באג שאותר בנתונים בפועל - HPR.xlsx): כשתת-הרכבה משותפת
    # בין כמה הורים/שורשים שונים (למשל אותה תת-הרכבה מופיעה גם תחת מוצר A
    # וגם תחת מוצר B), ASSEMBLY_BOM_TREE[pn]["parent"] הוא ערך *יחיד* -
    # נשמר רק ההורה מהמופע *האחרון* שנקרא בזמן טעינת הקובץ (הפריט הקודם
    # פשוט נדרס). לכן הליכה במעלה שרשרת ה-"parent" כדי "למצוא את השורש של
    # ההרכבה הזו" הייתה נופלת תמיד על אותו שורש קבוע אחד - גם כשההרכבה
    # שייכת בפועל לכמה שורשים - מה שגרם לתאריך/Slack/"גורם מגביל" שגויים
    # או חסרים כשההרכבה נצפית מתוך הקשר של השורש ה"אחר".
    # התיקון: בונים מיפוי *גלובלי* (לא לפי שורש) לפי PN בלבד, שסורק את כל
    # המופעים בכל העצים (של כל השורשים) ושומר עבור כל PN את המופע עם ה-EF
    # המאוחר ביותר (תרחיש הכי גרוע = הכי נכון להצגה - אותו עיקרון שכבר
    # היה נהוג כאן, רק עכשיו נכון גם כשההרכבה שייכת ליותר משורש אחד) -
    # יחד עם ה-root_pn שממנו המופע הזה בפועל הגיע, כדי שאפשר יהיה להמשיך
    # ולבנות ממנו את שרשרת הגורם המגביל (_cp_limiting_chain_str) בלי לסמוך
    # בכלל על שדה ה-"parent" השגוי.
    _cp_global_by_pn = {}  # asm_pn -> {"node": node, "root": root_pn}
    for _root, _res in _cp_results.items():
        for _node in _res["nodes"].values():
            if _node["kind"] == "assembly":
                _prev = _cp_global_by_pn.get(_node["pn"])
                if _prev is None or _node["ef"] > _prev["node"]["ef"]:
                    _cp_global_by_pn[_node["pn"]] = {"node": _node, "root": _root}

    def _cp_find_node_for_assembly(asm_pn):
        """מוצא את הצומת המייצג (התרחיש הכי גרוע, בכל שורש שהוא מופיע בו)
        של הרכבה נתונה - חיפוש גלובלי, לא תלוי בשדה "parent" היחיד/השגוי."""
        _entry = _cp_global_by_pn.get(asm_pn)
        return _entry["node"] if _entry else None

    def _cp_find_root_for_assembly(asm_pn):
        """מחזיר את ה-root_pn שממנו הגיע הצומת המייצג (הכי גרוע) של ההרכבה -
        לשימוש ב-_cp_limiting_chain_str, שצריכה לדעת מתוך איזה עץ/שורש
        לשחזר את השרשרת. תיקון לאותו באג - בלי הליכה בשרשרת "parent"."""
        _entry = _cp_global_by_pn.get(asm_pn)
        return _entry["root"] if _entry else None

    def _eta_note_for(node):
        """מחזיר הערת טקסט קצרה (עם רווח מוביל) שמסבירה על מה מבוסס ה-EF -
        מבדילה בין "אין ETA בכלל" לבין "היה ETA אבל הוא כבר עבר" (ראה
        _effective_component_date) - כדי שהמשתמש יידע במפורש איזה משני
        המקרים קרה, ולא רק ש"זו הנחה"."""
        _reason = node.get("availability_reason")
        if _reason == "eta_expired":
            return " (ETA עבר - מטופל כאילו אין ETA)"
        if node.get("is_eta_assumption"):
            return " (הנחה - אין ETA)"
        return ""

    def _cp_limiting_chain_str(node_id, root_pn, max_depth=12):
        """בונה מחרוזת שמראה בדיוק איזו שרשרת של רכיבים/תתי-הרכבות קובעת את
        תאריך המוכנות של הצומת הזה - יורדים מהצומת כלפי מטה, ובכל שלב עוברים
        לילד עם ה-EF המאוחר ביותר (זה בדיוק הילד שקבע את ה-ES של ההורה, ולכן
        הוא ה"אשם" בעיכוב) - עד לרכיב-הקצה שבסוף השרשרת, שהוא בדיוק הפריט
        שצריך לבדוק/לזרז כדי לקצר את התאריך.

        חריג חשוב: אם רכיב-הקצה בסוף השרשרת זמין במלאי עכשיו (availability_reason
        == "stock") - הוא לא באמת "אשם" בשום דבר; הוא רק נבחר כי היה צריך
        לבחור *מישהו* מבין רכיבים שכולם זמינים באותה מידה. במקרה כזה מציינים
        זאת במפורש, כדי לא ליצור רושם מטעה שרכיב עם מלאי עצום "חוסם" - התאריך
        במקרה הזה נובע רק ממספר שכבות ההרכבה בעץ (2 ימים לכל שכבה)."""
        _res = _cp_results[root_pn]
        _nodes = _res["nodes"]
        _parts = []
        _nid = node_id
        _depth = 0
        while _nid is not None and _depth < max_depth:
            _node = _nodes[_nid]
            if _node["kind"] == "component":
                if _node.get("availability_reason") == "stock":
                    _parts.append(f"אין רכיב חוסם - {_node['pn']} זמין במלאי עכשיו; התאריך נובע רק ממספר שכבות ההרכבה")
                else:
                    _parts.append(f"{_node['pn']} — {str(_node['desc'])[:30]}{_eta_note_for(_node)}")
                break
            if not _node["children_ids"]:
                _parts.append(f"{_node['pn']} (אין רכיבים/תתי-הרכבות מזוהים)")
                break
            _parts.append(_node["pn"])
            _nid = max(_node["children_ids"], key=lambda cid: _nodes[cid]["ef"])
            _depth += 1
        return " ← ".join(_parts) if _parts else "—"

    # ---------- טבלת סיכום ----------
    _cp_rows = []
    for _asm in valid_assemblies:
        _node = _cp_find_node_for_assembly(_asm)
        if _node is None:
            _cp_rows.append({
                "קוד הרכבה": _asm, "תיאור": ASSEMBLY_BOM_TREE.get(_asm, {}).get("desc", ""),
                "רמה בעץ": ASSEMBLY_BOM_TREE.get(_asm, {}).get("level", 0),
                "על נתיב החסימה?": "לא", "תאריך מוכנות (נתיב קריטי)": "—",
                "_sort_date": date.today(), "מבוסס על הנחה": "—", "Slack (ימים)": "—",
                "הגורם המגביל בשרשרת": "—",
            })
            continue
        _root_of_asm = _cp_find_root_for_assembly(_asm)
        _date_str = _node["ef"].strftime("%Y-%m-%d") + _eta_note_for(_node)
        _cp_rows.append({
            "קוד הרכבה": _asm,
            "תיאור": ASSEMBLY_BOM_TREE.get(_asm, {}).get("desc", ""),
            "רמה בעץ": ASSEMBLY_BOM_TREE.get(_asm, {}).get("level", 0),
            "על נתיב החסימה?": "🔴 כן" if _node["is_critical"] else "לא",
            "תאריך מוכנות (נתיב קריטי)": _date_str,
            "_sort_date": _node["ef"],
            "מבוסס על הנחה": (
                "כן (ETA עבר)" if _node.get("availability_reason") == "eta_expired"
                else ("כן (הנחה)" if _node.get("is_eta_assumption") else "לא (ETA אמיתי)")
            ),
            "Slack (ימים)": int(_node["slack_days"]),
            "הגורם המגביל בשרשרת": _cp_limiting_chain_str(_node["id"], _root_of_asm),
        })

    _cp_df = pd.DataFrame(_cp_rows).sort_values(["רמה בעץ", "_sort_date"], ascending=[True, False]).drop(columns=["_sort_date"])

    def _cp_highlight(val):
        if isinstance(val, str) and ("הנחה" in val or "🔴" in val):
            return f"background-color: {WARNING}; color: white; font-weight: 700;" if "הנחה" in val else f"background-color: {DANGER}; color: white; font-weight: 700;"
        return ""

    # מיפוי PN -> נתוני חוסר, מתוך breakdown_df - **בדיוק** אותה טבלה ששאר
    # האפליקציה (כולל הכרטיס העליון "אקסקיוטיב") מציגה כ"רשימת החוסרים"
    # בפועל, כדי שהעמודה החדשה למטה תהיה עקבית ל-100% עם מה שמוצג שם.
    # דה-דופ לפי PN (drop_duplicates) כי אותו PN יכול לחזור על עצמו כמה
    # פעמים ב-breakdown_df - פעם לכל הרכבה שצורכת אותו - אבל Total_MRP_
    # Shortage/Shortage_Value הם כבר ערך-לרכיב (לא לפי הרכבה), אז השורה
    # הראשונה מספיקה.
    _shortage_lookup = {}
    if not breakdown_df.empty:
        for _, _brow in breakdown_df.drop_duplicates(subset=["PN"]).iterrows():
            _shortage_lookup[str(_brow["PN"]).strip()] = {
                "qty": safe_num(_brow.get("Total_MRP_Shortage", 0.0)),
                "value": safe_num(_brow.get("Shortage_Value", 0.0)),
            }

    # ==========================================================
    # טבלת "סדר עדיפויות להאצה" - לכל שורש (מוצר ברמה 0) בנפרד: כל רכיבי-
    # הקצה שבאמת חוסמים (לא רכיבים שכבר זמינים במלאי - אין טעם "להאיץ"
    # רכיב שכבר יש ממנו מלאי), ממוינים מ-Slack הקטן ביותר (0 = הנתיב הקריטי
    # בפועל, זה שקובע את התאריך הנוכחי) לגדול ביותר. זה בדיוק "הנתיבים
    # המשניים" ש-CPM קלאסי מגדיר: אם מקדימים את הפריט בדרגה 1 (Slack=0)
    # ב-N ימים, הפריט הבא ברשימה שה-Slack שלו קטן מ-N יהפוך לנתיב הקריטי
    # החדש - ולכן זה בדיוק סדר העדיפות הנכון להתמקדות בפועל, פריט אחרי פריט.
    #
    # הערה חשובה (מגבלה ביודעין, לא "טעות"): הדירוג מבוסס על ה-Slack כפי
    # שחושב *עכשיו*, בתמונת מצב אחת - הוא לא מדמה מחדש את כל העץ אחרי כל
    # "האצה" היפותטית של הפריט הקודם ברשימה (שינוי אמיתי בתאריך זמינות של
    # רכיב עשוי תיאורטית לשנות גם slack-ים של רכיבים אחרים בענפים שמושפעים
    # ממנו). זה קירוב טוב ומקובל ב-CPM קלאסי (לא רה-סימולציה דינמית מלאה),
    # ומספיק בהחלט כדי לדעת באיזה פריט להתמקד קודם.
    #
    # דה-דופ: אם אותו PN מופיע ביותר ממקום אחד בעץ של אותו שורש (רכיב
    # משותף בין כמה ענפים בתוך אותו מוצר) - נשמר המופע עם ה-Slack הקטן
    # ביותר (הכי דחוף), כי זה בדיוק מה שקובע בפועל מתי חובה להקדים אותו.
    # ==========================================================
    _priority_rows_by_root = {}
    for _root in _cp_roots:
        _res = _cp_results[_root]
        _root_target_date = _res["nodes"][_res["root_id"]]["ef"]
        _root_desc = _res["nodes"][_res["root_id"]]["desc"]
        _by_pn = {}
        for _node in _res["nodes"].values():
            if _node["kind"] != "component":
                continue
            if _node.get("availability_reason") == "stock":
                continue  # כבר זמין במלאי - אין מה "להאיץ" בו
            _prev = _by_pn.get(_node["pn"])
            if _prev is None or _node["slack_days"] < _prev["slack_days"]:
                _by_pn[_node["pn"]] = _node
        _rows = []
        for _pn, _node in sorted(_by_pn.items(), key=lambda kv: (kv[1]["slack_days"], kv[1]["ef"])):
            _short_info = _shortage_lookup.get(str(_pn).strip())
            _shortage_col = (
                f"🔴 כן - {_short_info['qty']:,.0f} יח' בחוסר (₪{_short_info['value']:,.0f})"
                if _short_info else "לא נמצא בטווח החודשים שנבחר בטאב אקסקיוטיב"
            )
            _rows.append({
                "מק\"ט רכיב": _pn,
                "תיאור": _node["desc"],
                "תאריך זמינות הרכיב (EF)": _node["ef"].strftime("%Y-%m-%d") + _eta_note_for(_node),
                "Slack (ימים)": int(_node["slack_days"]),
                "על הנתיב הקריטי?": "🔴 כן" if _node["is_critical"] else "לא",
                "מוצר-שורש (רמה 0) שמושפע": f"{_root} ({_root_desc})",
                "תאריך יעד נוכחי של המוצר": _root_target_date.strftime("%Y-%m-%d"),
                "משותף גם לשורש אחר?": _shared_root_note(_node, _root).strip(" ⚠️") or "לא",
                "מופיע ברשימת החוסרים (טאב אקסקיוטיב)?": _shortage_col,
            })
        if _rows:
            for _i, _r in enumerate(_rows, start=1):
                _r["דירוג עדיפות"] = _i
            _priority_rows_by_root[_root] = pd.DataFrame(_rows)[
                ["דירוג עדיפות", "מק\"ט רכיב", "תיאור", "תאריך זמינות הרכיב (EF)", "Slack (ימים)",
                 "על הנתיב הקריטי?", "מוצר-שורש (רמה 0) שמושפע", "תאריך יעד נוכחי של המוצר",
                 "משותף גם לשורש אחר?", "מופיע ברשימת החוסרים (טאב אקסקיוטיב)?"]
            ]

    def _priority_highlight(val):
        if isinstance(val, str) and (val == "🔴 כן" or val.startswith("🔴 כן")):
            return f"background-color: {DANGER}; color: white; font-weight: 700;"
        return ""

    tab_tree, tab_table, tab_priority = st.tabs(["🌳 תצוגת עץ נפתחת", "📋 טבלת סיכום", "🔢 סדר עדיפויות להאצה"])

    with tab_tree:
        st.caption("🔴 = ההרכבה הזו (או שרשרת מתחתיה) קובעת בפועל את תאריך המוכנות של ראש העץ. Slack=0 ימים = על הנתיב הקריטי. לחצו על כל שורה כדי לפתוח/לסגור את הרמה הבאה.")

        # ---------- דריל-דאון מודגש: השרשרת המלאה מהשורש עד לרכיב שבאמת
        # "אשם" בתאריך - בלי צורך לפתוח expanders בשלבים. משתמשים ישירות
        # ב-critical_path שכבר בנויה בסדר הנכון (שורש -> עלה) בתוך
        # calculate_critical_path (backtracking לפי ה-EF המאוחר ביותר בכל
        # שלב) - הצומת האחרון ברשימה הוא תמיד רכיב-הקצה שבאמת קובע את
        # התאריך, ומודגש בבירור (צבע/מסגרת/תווית 🎯) ביחס לשאר השרשרת.
        st.markdown("#### 🎯 Drill-down: השרשרת שקובעת את התאריך, מהשורש (רמה 0) ועד לרכיב שבאמת אשם")
        for _root in sorted(_cp_roots):
            _chain = _cp_results[_root]["critical_path"]
            if len(_chain) < 2:
                continue
            _root_desc = _chain[0]["desc"]
            _last_is_stock = _chain[-1].get("availability_reason") == "stock"
            with st.expander(f"🔗 {_root} — {_root_desc}  |  יעד: {_chain[0]['ef'].strftime('%Y-%m-%d')}", expanded=True):
                if _last_is_stock:
                    st.info(
                        f"אין רכיב שחוסם בפועל בשרשרת הזו - כל הרכיבים בענף הזה זמינים במלאי עכשיו. "
                        f"התאריך {_chain[0]['ef'].strftime('%Y-%m-%d')} נובע אך ורק ממספר שכבות ההרכבה בעץ "
                        f"({len(_chain) - 1} שכבות × 2 ימי הרכבה = {(len(_chain) - 1) * 2} ימים), לא ממחסור בחומר."
                    )
                for _i, _cnode in enumerate(_chain):
                    _is_last = (_i == len(_chain) - 1)
                    _cdate = _cnode["ef"].strftime("%Y-%m-%d") + _eta_note_for(_cnode)
                    _kind_label = "רכיב-קצה (רכש/חומר גלם)" if _cnode["kind"] == "component" else "הרכבה"
                    _shared_note = _shared_root_note(_cnode, _root)
                    if _is_last and _last_is_stock:
                        # לא צובעים אדום - זה לא באמת "אשם", יש לו מלאי. צבע
                        # ניטרלי/ירוק עם תווית מפורשת שמסבירה את זה.
                        st.markdown(
                            f"<div style='margin:2px 0 2px {_i * 14}px; padding:9px 14px; background:{SUCCESS}; color:white; "
                            f"border-radius:8px; font-weight:700; box-shadow:0 2px 6px rgba(0,0,0,0.15);'>"
                            f"✅ {_cnode['pn']} — {_cnode['desc']}<br>"
                            f"<span style='font-weight:400; font-size:12px;'>{_kind_label} | זמין במלאי עכשיו - לא חוסם בפועל{_shared_note}</span>"
                            f"</div>",
                            unsafe_allow_html=True
                        )
                        continue
                    _icon = "🎯" if _is_last else "🔴"
                    _tag = " | זהו הגורם שבפועל קובע את התאריך הסופי" if _is_last else ""
                    # כל צומת בשרשרת (מרמה 0 ועד לרכיב-הקצה) צבוע אדום מלא -
                    # כל השרשרת כולה, לא רק הרכיב האחרון, כדי שאפשר יהיה
                    # לעקוב חזותית מלמעלה עד למטה בלי לפספס אף חוליה.
                    st.markdown(
                        f"<div style='margin:2px 0 2px {_i * 14}px; padding:9px 14px; background:{DANGER}; color:white; "
                        f"border-radius:8px; font-weight:{800 if _is_last else 600}; box-shadow:0 2px 6px rgba(0,0,0,0.20);'>"
                        f"{_icon} {_cnode['pn']} — {_cnode['desc']}<br>"
                        f"<span style='font-weight:400; font-size:12px;'>{_kind_label} | מוכנות: {_cdate}{_tag}{_shared_note}</span>"
                        f"</div>",
                        unsafe_allow_html=True
                    )
                    if not _is_last:
                        st.markdown(f"<div style='margin-right:{_i * 14 + 12}px; color:{DANGER}; font-weight:700;'>↓</div>", unsafe_allow_html=True)

        st.markdown("---")
        st.markdown("#### 🌳 עץ מלא (כל הענפים, לא רק הנתיב הקריטי)")

        _cp_root_blocker_id = {_r: _cp_results[_r]["critical_path"][-1]["id"] for _r in _cp_roots if len(_cp_results[_r]["critical_path"]) >= 1}

        def _render_cp_node(node_id, root_pn, depth):
            _res = _cp_results[root_pn]
            node = _res["nodes"][node_id]
            is_critical = node["is_critical"]
            is_the_blocker = (node_id == _cp_root_blocker_id.get(root_pn))
            date_str = node["ef"].strftime("%Y-%m-%d") + _eta_note_for(node)
            icon = "🎯" if is_the_blocker else ("🔴" if is_critical else "⚪")
            slack_txt = f" | Slack: {int(node['slack_days'])}י'" if not is_critical else ""
            label = f"{icon} {node['pn']} — {node['desc']}  |  מוכנות: {date_str}{slack_txt}{_shared_root_note(node, root_pn)}"
            children_ids = sorted(node["children_ids"], key=lambda cid: 0 if _res["nodes"][cid]["is_critical"] else 1)

            if children_ids:
                with st.expander(label, expanded=(depth == 0 or is_critical)):
                    for cid in children_ids:
                        _render_cp_node(cid, root_pn, depth + 1)
            else:
                color = DANGER if is_critical else "inherit"
                weight = 700 if is_critical else 400
                bg = f"background:{DANGER}; color:white; border-radius:6px; padding:8px 10px;" if is_the_blocker else ""
                st.markdown(
                    f"<div style='padding:5px 10px; margin-bottom:3px; color:{color}; font-weight:{weight}; {bg}"
                    f"border-right:3px solid {DANGER if is_critical else 'rgba(128,128,128,0.3)'};'>{label}</div>",
                    unsafe_allow_html=True
                )

        for _root in sorted(_cp_roots):
            _render_cp_node(_cp_results[_root]["root_id"], _root, 0)

    with tab_table:
        st.dataframe(
            _cp_df.style.map(_cp_highlight, subset=["תאריך מוכנות (נתיב קריטי)", "מבוסס על הנחה", "על נתיב החסימה?"]),
            use_container_width=True, height=460
        )

        _top_level = _cp_df[_cp_df["רמה בעץ"] == 0]
        if not _top_level.empty:
            _chart_df = _cp_df[_cp_df["תאריך מוכנות (נתיב קריטי)"] != "—"].copy()
            # תיקון: הסרה גנרית של כל הערה בסוגריים בסוף המחרוזת (לא רק
            # "(הנחה)") - כי מאז ההוספה של הערת "(ETA עבר...)" יכולות
            # להופיע כמה הערות שונות בסוף התאריך, וההסרה הקודמת (טקסט קבוע
            # בלבד) הייתה משאירה אותן בפנים ומפילה את pd.to_datetime.
            _chart_df["_dt"] = pd.to_datetime(_chart_df["תאריך מוכנות (נתיב קריטי)"].str.replace(r"\s*\(.*\)\s*$", "", regex=True))
            fig_cp = px.bar(
                _chart_df.sort_values("_dt"), x="_dt", y="קוד הרכבה", orientation="h",
                color="מבוסס על הנחה", color_discrete_map={"כן (הנחה)": WARNING, "לא (ETA אמיתי)": SUCCESS},
                hover_data=["תיאור", "Slack (ימים)"],
                labels={"_dt": "תאריך מוכנות (נתיב קריטי)", "קוד הרכבה": ""}
            )
            fig_cp.update_layout(template=PLOTLY_TEMPLATE, height=max(400, 26 * len(_chart_df)), margin=dict(t=10, b=10, l=10, r=10))
            st.plotly_chart(fig_cp, use_container_width=True)

    with tab_priority:
        st.caption(
            "לכל מוצר-שורש (רמה 0) בנפרד: רכיבי-הקצה שבאמת חוסמים (לא כאלה שכבר זמינים במלאי), ממוינים "
            "מהדחוף ביותר (Slack=0 - זהו הנתיב הקריטי שקובע היום את תאריך היעד) לפחות דחוף. הרעיון: אם "
            "מקדימים בפועל את פריט מס' 1, ורווח ה-Slack שלו (העמודה הבאה) מכסה את מה שהקדמתם - פריט מס' 2 "
            "הופך להיות זה שקובע את התאריך, וכן הלאה. עמודת \"מופיע ברשימת החוסרים\" מציינת אילו מהפריטים "
            "האלה מופיעים גם בפועל בטבלת החוסרים של הטאב האקסקיוטיבי (עם כמות/ערך החוסר משם) - כלומר לא רק "
            "\"עיכוב תיאורטי בציר הזמן\" אלא חוסר ממשי שכבר נספר שם. ⚠️ זהו קירוב על בסיס מצב הנתונים "
            "הנוכחי - אינו מדמה מחדש את כל העץ אחרי כל הקדמה היפותטית, אבל נותן סדר עבודה ברור למי לפנות קודם."
        )
        if not _priority_rows_by_root:
            st.info("לא נמצאו רכיבים חוסמים (מבוססי ETA) בשום מוצר-שורש - כל הרכיבים זמינים במלאי כרגע.")
        else:
            for _root in sorted(_priority_rows_by_root.keys()):
                _pdf = _priority_rows_by_root[_root]
                _root_desc = _cp_results[_root]["nodes"][_cp_results[_root]["root_id"]]["desc"]
                _root_target = _cp_results[_root]["nodes"][_cp_results[_root]["root_id"]]["ef"].strftime("%Y-%m-%d")
                with st.expander(f"🔗 {_root} — {_root_desc}  |  יעד נוכחי: {_root_target}  |  {len(_pdf)} רכיבים חוסמים", expanded=True):
                    st.dataframe(
                        _pdf.style.map(_priority_highlight, subset=["על הנתיב הקריטי?", "מופיע ברשימת החוסרים (טאב אקסקיוטיב)?"]),
                        use_container_width=True, height=min(460, 46 * (len(_pdf) + 1)), hide_index=True
                    )
