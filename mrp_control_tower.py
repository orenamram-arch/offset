...
MISSING_ETA_DEFAULT_MONTHS = 2         # הנחת ברירת מחדל לרכיב בלי ETA: today + חודשיים
...
def calculate_critical_path(root_asm_pn, assembly_children=None, assembly_direct_components=None,
    ...
    שיטת החישוב - CPM קלאסי, שני מעברים על העץ:
      1. Forward Pass (Post-order - הילדים תמיד מחושבים לפני ההורה):
         - צומת-רכיב (עלה, רכש): ES=today, EF=ETA האפקטיבי (או
           today+relativedelta(months=2) אם אין ETA תקין/הרכיב "בדיקה נדרשת").
         - צומת-הרכבה: ES = המקסימום (המאוחר) מבין EF של כל הרכיבים הישירים
           ותתי-ההרכבות שלו (כולם חייבים להיות מוכנים לפני שאפשר להתחיל
           להרכיב). EF = ES + 2 ימים (משך הרכבה קבוע).
    ...
    is_eta_assumption (True אם EF מבוסס על ברירת המחדל של 2 חודשים, לא ETA אמיתי).
    ...
    def _effective_component_date(comp_pn):
        ...
        if eta_raw and str(eta_raw).strip() not in ("", "None", "NaT", "nan", "בדיקה נדרשת"):
            try:
                ...
                pass
        return today + relativedelta(months=MISSING_ETA_DEFAULT_MONTHS), True
...
""")
    ...
    st.caption(
        ...
        "רכיב בלי ETA מוגדר כלל מקבל הנחת ברירת מחדל של חודשיים מהיום, ומסומן בבירור \"(הנחה)\" בתוצאה."
    )
