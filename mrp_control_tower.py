...
""")
    ...
    _cp_shortage_pns = set(breakdown_df["PN"].unique()) if not breakdown_df.empty else set()
    st.caption(f"🔎 מבוסס על {len(_cp_shortage_pns)} רכיבים שנמצאים כרגע בחוסר בפועל בטווח/פרויקט הנבחר (מאותה רשימת חוסרים שמוצגת בשאר האפליקציה).")
