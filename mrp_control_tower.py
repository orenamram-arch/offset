...
def calculate_critical_path(root_asm_pn, assembly_children=None, assembly_direct_components=None,
    ...
    def _build(pn, parent_id, _path=frozenset()):
        ...
        for comp_pn, qty_per, comp_desc, base_stock in assembly_direct_components.get(pn, []):
            ...
            # בדיוק הדרך שבה אוכפים את הכלל "אין ETA עצמאי להרכבות" - אחרת
            # השורה הזו הייתה נכנסת לבדיקת ETA כאילו היא רכיב-קצה, בניגוד מוחלט
            # לכלל העסקי (וגם יוצרת כפילות עם הענף שכבר טופל כתת-הרכבה).
