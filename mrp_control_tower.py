...
# כתובת ה-raw.githubusercontent.com של הקובץ. אפשר גם להדביק קישור זמני
# ב-"קישור מותאם אישית" מבלי לערוך את הקוד בכלל.
#
...
with st.sidebar:
    ...
    _selected_project = st.selectbox("בחר פרויקט", _project_names, key="selected_project_name")
    if _selected_project == "🔗 קישור מותאם אישית...":
        ...
        ACTIVE_PROJECT_URL = st.text_input(
            "כתובת raw.githubusercontent.com לקובץ ה-Excel",
        ...
        ACTIVE_PROJECT_URL = AVAILABLE_PROJECTS[_selected_project]
...
def _find_matrix_header_row(raw_df):
    """מאתר את שורת הכותרת הראשית של הגיליון (העמודות '#' ו-'PN_ID' בהתחלה)."""
    for r in range(raw_df.shape[0]):
        v0, v1 = raw_df.iat[r, 0], raw_df.iat[r, 1]
        if (isinstance(v0, str) and v0.strip() == "#") and (isinstance(v1, str) and v1.strip().upper() == "PN_ID"):
            return r
    return None
...
def _find_desc_level_pn_cols(raw_df):
    """מאתר בכל הגיליון שלישיית עמודות סמוכות עם הכותרות 'DESC','LEVEL','PN' - זהו בלוק
    עץ ה-BOM המפורק. גם מזהה אם יש עמודת כמות ('QTY...') מיד אחרי PN."""
    max_r, max_c = raw_df.shape
    for r in range(max_r):
        for c in range(max_c - 2):
            v0, v1, v2 = raw_df.iat[r, c], raw_df.iat[r, c + 1], raw_df.iat[r, c + 2]
            if (isinstance(v0, str) and v0.strip().upper() == "DESC" and
...
def _longest_date_run_ending_before(named_cols, end_idx_exclusive, lookback=60):
...
PN_COL = df.columns[1]  # '#','PN_ID' תמיד שתי העמודות הראשונות (מאומת ע"י find_matrix_header_row)
...
PRICE_COL = _find_column_by_candidates(df.columns, ["PRICE_CALC", "TARGET PRICE", "TOTAL PRICE", "PRICE"])
AW_COL = _find_column_by_candidates(df.columns, ["ספק", "SUPPLIER", "MANUFACTURER", "POC SUPPLIER"])
...
if _dlp is None:
    st.error("לא אותר בלוק 'DESC/LEVEL/PN' בקובץ - לא ניתן לפרוס את עץ ה-BOM עבור הפרויקט הזה.")
else:
    try:
        ...
        for _r, _pn, _level, _desc, _qty_raw in _rows:
            ...
            if _qty_c is not None:
                _qty_per_system = safe_num(_qty_raw, default=1.0)
                _qty_per_parent = _qty_per_system / _parent_sys_qty if _parent_sys_qty > 0 else 1.0
            else:
                _qty_per_system, _qty_per_parent = None, None  # ימולא בהעשרה למטה מתוך המטריצה
        ...
        # העשרת כמויות מתוך מטריצת ה-where-used, רק כשלא הייתה עמודת כמות בבלוק העץ עצמו
        if _qty_c is None and ASSEMBLY_BOM_TREE:
            _df_pnid = df[PN_COL].astype(str).str.strip()
            for _pn in ordered_assemblies_from_excel:  # כבר בסדר טופולוגי - הורה לפני ילד
                _node = ASSEMBLY_BOM_TREE[_pn]
                _parent_pn = _node["parent"]
                if _parent_pn is None:
                    _node["qty_per_system"] = 1.0
                    _node["qty_per_parent"] = 1.0
                else:
                    _match_idx = _df_pnid[_df_pnid == _pn].index
                    _q = 1.0
                    if len(_match_idx) > 0 and _parent_pn in df.columns:
                        _q = safe_num(df.loc[_match_idx[0], _parent_pn], default=1.0)
                        if _q <= 0:
                            _q = 1.0
                    _node["qty_per_parent"] = _q
                    _node["qty_per_system"] = _q * ASSEMBLY_BOM_TREE[_parent_pn]["qty_per_system"]
    except Exception as e:
...
_plan_start_idx = (_dlp["qty"] + 1) if (_dlp and _dlp["qty"] is not None) else ((_dlp["pn"] + 1) if _dlp else None)
