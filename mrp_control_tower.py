...
""")
    ...
    if not _bom_positions:
        ...
        # קווי חיבור הורה-ילד + תוויות כמות-קשר, מצוירים לפני המלבנים
        # כדי שהמלבנים "יעלו" חזותית מעל הקווים.
        for pn, info in ASSEMBLY_BOM_TREE.items():
            parent = info.get("parent")
            if parent is None or parent not in _bom_positions or pn not in _bom_positions:
                continue
            px_, py_ = _bom_positions[parent], info.get("level", 0) - 1
            cx_, cy_ = _bom_positions[pn], info.get("level", 0)
            shapes.append(dict(
                type="line", x0=px_, y0=py_, x1=cx_, y1=cy_,
                line=dict(color="rgba(148,163,184,0.65)", width=1.6)
            ))
            qty_lbl = info.get("qty_per_parent", 1.0) or 1.0
            annotations.append(dict(
                x=(px_ + cx_) / 2, y=(py_ + cy_) / 2,
