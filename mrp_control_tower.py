...
""")
    ...
    with tab_tree:
        ...
        st.markdown("#### 🎯 דריל-דאון: השרשרת שקובעת את התאריך, מהשורש ועד לרכיב שבאמת אשם")
        for _root in sorted(_cp_roots):
            ...
            with st.expander(f"🔗 {_root} — {_root_desc}  |  יעד: {_chain[0]['ef'].strftime('%Y-%m-%d')}", expanded=True):
                for _i, _cnode in enumerate(_chain):
                    ...
                    if _is_last:
                        st.markdown(
                            f"<div style='margin:2px 0 2px 24px; padding:10px 14px; background:{DANGER}; color:white; "
                            f"border-radius:8px; font-weight:800; box-shadow:0 2px 6px rgba(0,0,0,0.25);'>"
                            f"🎯 {_cnode['pn']} — {_cnode['desc']}<br>"
                            f"<span style='font-weight:400; font-size:12px;'>{_kind_label} | מוכנות: {_cdate} | זהו הגורם שבפועל קובע את התאריך הסופי</span>"
                            f"</div>",
                            unsafe_allow_html=True
                        )
                    else:
                        _indent = 24 + _i * 18
                        st.markdown(
                            f"<div style='margin:2px 0 2px {_indent}px; padding:6px 12px; background:rgba(220,38,38,0.10); "
                            f"border-right:3px solid {DANGER}; border-radius:4px;'>"
                            f"<b>{_cnode['pn']}</b> — {_cnode['desc']} "
                            f"<span style='font-size:11px; opacity:0.75;'>({_kind_label}, מוכנות: {_cdate})</span>"
                            f"</div>",
                            unsafe_allow_html=True
                        )
                        st.markdown(f"<div style='margin-right:{_indent + 12}px; opacity:0.5;'>↓</div>", unsafe_allow_html=True)
