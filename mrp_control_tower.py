...
""")
    ...
    # תיקון קריטי: לא משתמשים ב-breakdown_df כמות שהוא - הוא מוגבל ל-
    # selected_target_yms (חלון "כמה חודשים קדימה" שנבחר בפילטרים של טאב
    # אחר!). רכיב עם חוסר אמיתי שמתגלה רק בחודש שמעבר לחלון הזה פשוט לא
    # מופיע ב-breakdown_df - והקוד היה מניח בטעות שהוא זמין כבר היום. נתיב
    # קריטי חייב לבדוק חוסרים על פני *כל* אופק התכנון (all_ym_list), בלי
    # תלות בפילטר של טאב אחר, ובלי הפילטרים הנוספים (סוג פריט/הרכבה/חיפוש)
    # שרלוונטיים רק לתצוגה האינטראקטיבית של טבלת החוסרים - לכן קוראים ישירות
    # ל-calculate_mrp_breakdown_cached (אותו מנגנון קאש בדיוק), לא ל-wrapper.
    _cp_full_breakdown_df = calculate_mrp_breakdown_cached(tuple(all_ym_list), tuple(), assembly_plan_df)
    _cp_shortage_pns = set(_cp_full_breakdown_df["PN"].unique()) if not _cp_full_breakdown_df.empty else set()
    st.caption(f"🔎 מבוסס על {len(_cp_shortage_pns)} רכיבים שנמצאים בחוסר אמיתי על פני כל אופק התכנון של הפרויקט (לא רק בחלון החודשים שנבחר בשאר האפליקציה).")
