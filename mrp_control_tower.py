...
if _plan_start_idx is not None and _demand_start_idx is not None and _demand_start_idx > _plan_start_idx:
    ...
    # אין תווית DEMAND - מחפשים את הרצף הרציף הראשון והמשמעותי (לפחות 3 חודשים
    # ברצף, כדי לא להיתפס לתא-תאריך בודד במקרה) של עמודות-תאריך, איפשהו אחרי
    # סוף עץ ה-BOM/המטריצה - לא בהכרח ממש בעמודה הבאה (בקבצים מסוימים יש פער
    # של עשרות עמודות של מטא-דאטה נוספת לפני שבלוק החודשים בפועל מתחיל).
    _min_run = 3
