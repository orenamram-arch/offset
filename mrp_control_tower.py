...
# גרסה מתוקנת ומואצת: סנכרון כמויות QPA ומפתחות מערכת (WIP / System Factor * QPA), 
# סידור עץ BOM דינמי מתוך האקסל, Caching מקיף למניעת סיבוכיות מעריכית, ותיקוני תצוגת UI עקבית.
...
# ==========================================================
# 🚀 אופטימיזציה: יצירת אינדקס מהיר לשליפת נתוני שורות לפי מק"ט 
# במקום לבצע df[df[PN_COL] == pn] בכל פעם מחדש.
# ==========================================================
DF_BY_PN = df.drop_duplicates(subset=[PN_COL], keep='first').set_index(df[PN_COL].astype(str).str.strip()).to_dict('index')

# 🚀 אינדקס מהיר עבור קובץ הגלם (df_raw) - מונע חיפושים ארוכים בחישוב ETA ו-Supply
raw_pn_col = df_raw.columns[1]
DF_RAW_BY_PN = df_raw.drop_duplicates(subset=[raw_pn_col], keep='first').set_index(df_raw[raw_pn_col].astype(str).str.strip()).to_dict('index')
...
def get_cumulative_incoming_supply(pn, target_ym):
    ...
    for col_pos, dt in SUPPLY_DATE_MAP.items():
        ...
        try:
            ym = dt.strftime("%Y-%m")
            if ym < target_ym:
                q = safe_num(row_dict.get(col_pos))
                if q > 0:
                    total += q
        except Exception:
            pass
...
def get_component_available_by_month(pn, target_ym, inv_cache=None, wip_cache=None):
    ...
    # 🚀 שימוש באינדקס המהיר במקום df[df[PN_COL] == pn]
    row_dict = DF_BY_PN.get(pn, None)
    base_stock = safe_num(row_dict.get(STOCK_COL, 0.0)) if row_dict else 0.0
    ...
    wip_committed = 0.0
    if row_dict and wip_cache:
        for asm_col in ASSEMBLY_COLS:
            wip_qty = wip_cache.get(asm_col, 0.0)
            if wip_qty > 0 and asm_col in row_dict:
                qty_per = safe_num(row_dict.get(asm_col, 0.0))
                if qty_per > 0:
...
def compute_shared_executable_plan(target_yms, assemblies, inv_cache=None, wip_cache=None):
    ...
    for ym in chronological_yms:
        pool_cache = {}
        global_memo = {} # 🚀 קריטי: זיכרון משותף לכל ההרכבות באותו חודש!

        def get_pool(pn):
            if pn not in pool_cache:
                if pn in ASSEMBLY_BOM_TREE:
                    sub_wip = wip_cache.get(pn, 0.0)
                    # 🚀 העברת ה-global_memo לפונקציה
                    sub_max_new, _ = compute_max_buildable(pn, ym, inv_cache, wip_cache, _memo=global_memo)
...
def check_hierarchical_ctb(asm_pn, requested_qty, target_ym, inv_cache=None, wip_cache=None, _visited=None, _claimed=None):
    ...
    if asm_pn in df.columns:
        ...
        # 🚀 אופטימיזציה: מסננים רק רכיבים שרלוונטיים להרכבה במקום לסרוק את כל הקובץ!
        relevant_components = df[pd.to_numeric(df[asm_pn], errors='coerce').fillna(0) > 0]
        
        for _, row in relevant_components.iterrows():
...
def _max_buildable_from_direct_components(asm_pn, target_ym, inv_cache, wip_cache):
    ...
    # 🚀 אופטימיזציה: מסננים רק שורות שרלוונטיות להרכבה הנוכחית מראש
    relevant_components = df[pd.to_numeric(df[asm_pn], errors='coerce').fillna(0) > 0]
...
def _max_buildable_from_subassemblies(asm_pn, target_ym, inv_cache, wip_cache, visited, memo):
    ...
    for child_pn in ASSEMBLY_CHILDREN.get(asm_pn, []):
        ...
        child_wip = wip_cache.get(child_pn, 0.0)
        # 🚀 העברת ה-memo גם לתתי ההרכבות
        child_max_new, _ = compute_max_buildable(child_pn, target_ym, inv_cache, wip_cache, visited, memo)
...
def compute_max_buildable(asm_pn, target_ym, inv_cache=None, wip_cache=None, _visited=None, _memo=None):
    ...
    # 🚀 בדיקה אם כבר חישבנו את ההרכבה הזו 
    if (asm_pn, target_ym) in _memo:
        return _memo[(asm_pn, target_ym)]
    ...
    # 🚀 שמירת התוצאה בזיכרון לפני חזרה
    result = (max(0.0, max_qty), limiting_component)
    _memo[(asm_pn, target_ym)] = result
